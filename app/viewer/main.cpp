...
// =============================================================================
// Thread-safe log buffer that also captures std::cout output
// =============================================================================
...
static const char *kStageNames[] = {
	"None",			  "1. Load Point Cloud", "2. FPS Downsample", "3. Laplacian Contraction", "4. FPS Downsample", "5. LOP Calibration",
...
// =============================================================================
// Distinct color palette for instance labels
// =============================================================================
static easy3d::vec3 label_color(int semantic, int instance) {
	if (semantic == 0)
		return { 0.55f, 0.27f, 0.07f };
	static const easy3d::vec3 pal[] = {
		{ 0.12f, 0.47f, 0.71f }, { 1.00f, 0.50f, 0.05f }, { 0.17f, 0.63f, 0.17f }, { 0.84f, 0.15f, 0.16f }, { 0.58f, 0.40f, 0.74f },
		{ 0.55f, 0.34f, 0.29f }, { 0.89f, 0.47f, 0.76f }, { 0.50f, 0.50f, 0.50f }, { 0.74f, 0.74f, 0.13f }, { 0.09f, 0.75f, 0.81f },
		{ 0.94f, 0.78f, 0.11f }, { 0.34f, 0.71f, 0.91f }, { 0.99f, 0.60f, 0.59f }, { 0.65f, 0.85f, 0.33f }, { 0.44f, 0.22f, 0.78f },
	};
	return pal[(instance >= 0 ? instance : 0) % 15];
}


// =============================================================================
// MyViewer  --  The main GUI application class
// =============================================================================
...
private:
	...
	bool show_param_panel_ = false;
	bool show_pipeline_panel_ = true;
	bool show_log_panel_ = true;
	bool show_layer_manager_ = true;
	...
	void draw_parameter_panel() {
		...
		if (ImGui::CollapsingHeader("Input Settings", ImGuiTreeNodeFlags_DefaultOpen)) {
			...
			bool batch = in.value("Batch_Processing", false);
			if (ImGui::Checkbox("Batch Processing", &batch))
				in["Batch_Processing"] = batch;

			if (batch) {
				ImGui::InputText("Batch Folder", buf_batch_dir_, sizeof(buf_batch_dir_));
				ImGui::SameLine();
				if (ImGui::SmallButton("...##bf")) {
					std::string d = easy3d::dialog::open_folder("Select Batch Folder");
					if (!d.empty())
						strncpy(buf_batch_dir_, d.c_str(), sizeof(buf_batch_dir_) - 1);
				}
				ImGui::InputText("File Extension", buf_ext_, sizeof(buf_ext_));
			} else {
			...
			if (wl) {
				if (ImGui::TreeNode("PLY Format Labels")) {
					auto &ply = in["Labels_Names"]["PLY_Format"];
					bool ind = ply.value("Individual_Labels_File", false);
					if (ImGui::Checkbox("Individual Files##ply", &ind))
						ply["Individual_Labels_File"] = ind;
					if (ind) {
						ImGui::InputText("Semantic File##ply", buf_sem_file_ply_, sizeof(buf_sem_file_ply_));
						ImGui::InputText("Instance File##ply", buf_inst_file_ply_, sizeof(buf_inst_file_ply_));
					} else {
						ImGui::InputText("Semantic Name", buf_sem_name_, sizeof(buf_sem_name_));
						ImGui::InputText("Instance Name", buf_inst_name_, sizeof(buf_inst_name_));
					}
					ImGui::TreePop();
				}
				if (ImGui::TreeNode("TXT/XYZ Format Labels")) {
					auto &txt = in["Labels_Names"]["TXT_XYZ_Format"];
					bool ind = txt.value("Individual_Labels_File", false);
					if (ImGui::Checkbox("Individual Files##txt", &ind))
						txt["Individual_Labels_File"] = ind;
					if (ind) {
						ImGui::InputText("Semantic File##txt", buf_sem_file_txt_, sizeof(buf_sem_file_txt_));
						ImGui::InputText("Instance File##txt", buf_inst_file_txt_, sizeof(buf_inst_file_txt_));
					} else {
						int tc = txt.value("Total_Columns", 0);
						if (ImGui::InputInt("Total Columns", &tc))
							txt["Total_Columns"] = tc;
						int si = txt.value("Semantic_Labels_Index", 0);
						if (ImGui::InputInt("Semantic Index", &si))
							txt["Semantic_Labels_Index"] = si;
						int ii = txt.value("Instance_Labels_Index", 0);
						if (ImGui::InputInt("Instance Index", &ii))
							txt["Instance_Labels_Index"] = ii;
					}
					ImGui::TreePop();
				}
			}
		...
		// ---- Constraint Laplacian Operator ----
		if (ImGui::CollapsingHeader("Constraint Laplacian Operator")) {
			auto &cl = cfg_["Constraint_Laplacian_Operator"];

			bool knn = cl.value("Use_KNN_Search", false);
			bool rad = cl.value("Use_Radius_Search", true);
		...
		if (ImGui::CollapsingHeader("Adaptive Contraction")) {
			...
			if (ImGui::SliderFloat("Sigma Threshold", &st, 0.0f, 1.0f, "%.3f"))
	...
	void draw_pipeline_panel() {
		...
		step_btn("2. FPS Downsample", Stage::FPS_DOWNSAMPLE_0, Stage::LOAD_POINT_CLOUD);
		step_btn("3. Laplacian Contraction", Stage::LAPLACIAN, Stage::FPS_DOWNSAMPLE_0);
		step_btn("4. FPS Downsample", Stage::FPS_DOWNSAMPLE_1, Stage::LAPLACIAN);
		step_btn("5. LOP Calibration", Stage::LOP_CALIBRATION, Stage::FPS_DOWNSAMPLE_1);
		...
		step_btn("9. Segment Skeleton", Stage::SEGMENT, Stage::PRUNE_MST);
		step_btn("10.Assign Labels", Stage::ASSIGN_LABELS, Stage::SEGMENT);
	...
	void ensure_output_dir() {
		...
		std::filesystem::create_directories(out / ".iterations");
	}
	...
	void exec(Stage stg) {
		...
		switch (stg) {
			case Stage::LOAD_POINT_CLOUD: {
				...
				tool::utility::Normalize(original_cloud_, cfg_["Preprocess"]["Normalize_AABB_Length"].get<double>());
				g_log.add("[INFO] Loaded " + std::to_string(original_cloud_.rows()) + " points");
				break;
			}
			case Stage::FPS_DOWNSAMPLE_0: {
				g_log.add("[INFO] FPS downsampling...");
	...
	void visualize(Stage stg) {
		switch (stg) {
			case Stage::LOAD_POINT_CLOUD:
				add_pc("Input Point Cloud", original_cloud_, { 0.6f, 0.6f, 0.6f });
				break;
			case Stage::FPS_DOWNSAMPLE_0:
				add_pc("FPS Downsampled Point Cloud", input_cloud_, { 0.6f, 0.6f, 0.6f });
			case Stage::LAPLACIAN:
				add_pc("Laplacian Skeleton", lap_skeleton_, { 1.0f, 0.2f, 0.2f });
				break;
			case Stage::FPS_DOWNSAMPLE_1:
				add_pc("FPS Downsampled Skeleton", fps_points_, { 0.2f, 1.0f, 0.2f });
				break;
			case Stage::LOP_CALIBRATION:
				add_pc("LOP Calibrated", lop_points_, { 0.2f, 0.2f, 1.0f });
				break;
			case Stage::BUILD_GRAPH:
				add_boost_graph("Initial Graph", initial_graph_);
				break;
			case Stage::COMPUTE_MST:
				add_boost_graph("MST", mst_graph_);
				break;
			case Stage::PRUNE_MST:
				add_boost_graph("Pruned MST", pruned_mst_);
				break;
			case Stage::SEGMENT:
				add_labeled_graph("Segmented Skeleton", pruned_mst_, skel_sem_, skel_inst_);
				break;
			case Stage::ASSIGN_LABELS:
				add_labeled_pc("Result", input_cloud_, pred_sem_, pred_inst_);
				break;
			default:
				break;
		}
	...
	void add_pc(const std::string &name, const Eigen::MatrixXd &m, easy3d::vec3 color) {
		if (m.rows() == 0) {
			return;
		}
		...
		for (int i = 0; i < m.rows(); ++i)
			pc->add_vertex(easy3d::vec3(static_cast<float>(m(i, 0)), static_cast<float>(m(i, 1)), static_cast<float>(m(i, 2))));
		pc->vertex_property<easy3d::vec3>("v:color", color);
		add_model(pc);
		fit_screen();
	}

	void add_boost_graph(const std::string &name, const Boost_Graph &bg) {
		if (boost::num_vertices(bg) == 0) {
			return;
		}
		auto *eg = new easy3d::Graph;
		eg->set_name(name);

		std::map<Boost_Vertex, easy3d::Graph::Vertex> vm;
		Boost_VertexIt vi, ve;
		for (boost::tie(vi, ve) = boost::vertices(bg); vi != ve; ++vi) {
			const auto &p = bg[*vi];
			vm[*vi] = eg->add_vertex(easy3d::vec3(static_cast<float>(p[0]), static_cast<float>(p[1]), static_cast<float>(p[2])));
		}
		Boost_EdgeIt ei, ee;
		for (boost::tie(ei, ee) = boost::edges(bg); ei != ee; ++ei)
			eg->add_edge(vm[boost::source(*ei, bg)], vm[boost::target(*ei, bg)]);

		add_model(eg);
		fit_screen();
	}

	void add_labeled_graph(const std::string &name, const Boost_Graph &bg, const std::vector<int> &sem, const std::vector<int> &inst) {
		if (boost::num_vertices(bg) == 0) {
			return;
		}
		auto *eg = new easy3d::Graph;
		...
		auto colors = eg->vertex_property<easy3d::vec3>("v:color", easy3d::vec3(0.5f, 0.5f, 0.5f));
		int idx = 0;
		for (boost::tie(vi, ve) = boost::vertices(bg); vi != ve; ++vi, ++idx)
			if (idx < static_cast<int>(sem.size()))
				colors[vm[*vi]] = label_color(sem[idx], inst[idx]);

		add_model(eg);
		fit_screen();
	}

	void add_labeled_pc(const std::string &name, const Eigen::MatrixXd &m, const std::vector<int> &sem, const std::vector<int> &inst) {
		if (m.rows() == 0) {
			return;
		}
		auto *pc = new easy3d::PointCloud;
		...
		auto colors = pc->vertex_property<easy3d::vec3>("v:color", easy3d::vec3(0.5f, 0.5f, 0.5f));
		for (int i = 0; i < m.rows(); ++i) {
			if (i < static_cast<int>(sem.size())) {
				colors[easy3d::PointCloud::Vertex(i)] = label_color(sem[i], inst[i]);
			}
		}
		add_model(pc);
		fit_screen();
