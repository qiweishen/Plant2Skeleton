	...
	namespace surface {
		namespace {
			...
			double computeOmega(IntrinsicGeometryInterface& geometry, const VertexData<Vector2>& directionField,
				...
				double omegaIJ =
						(lij / 2.) * (frequencies[e.firstVertex()] * cos(phiI - thetaI) + frequencies[e.secondVertex()] * cos(phiJ - thetaJ));

				return omegaIJ;
			}

			// Build a Laplace-like matrix with double entries (necessary to represent complex conjugation)
			SparseMatrix<double> buildVertexEnergyMatrix(IntrinsicGeometryInterface& geometry, const VertexData<Vector2>& directionField,
			...
			VertexData<Vector2> computeParameterization(IntrinsicGeometryInterface& geometry, const VertexData<Vector2>& directionField,
														const FaceData<int>& branchIndices, const VertexData<double>& frequencies) {
				SurfaceMesh& mesh = geometry.mesh;
		...
		std::tuple<CornerData<double>, FaceData<int>, FaceData<int>> computeStripePattern(IntrinsicGeometryInterface& geometry,
																						  const VertexData<double>& frequencies,
																						  const VertexData<Vector2>& directionField) {
			...
			VertexData<Vector2> parameterization = computeParameterization(geometry, directionField, branchIndices, 2 * PI * frequencies);
			...
			std::tie(textureCoordinates, zeroIndices) = computeTextureCoordinates(geometry, directionField, 2 * PI * frequencies, parameterization);
		...
		namespace {
			...
			std::tuple<std::vector<Vector3>, std::vector<std::array<size_t, 2>>, EdgeData<std::vector<size_t>>> extractCrossingsFromStripePattern(
					EmbeddedGeometryInterface& geometry, const CornerData<double>& stripeValues, const FaceData<int>& stripeIndices,
				...
				for (Face f: mesh.faces()) {
					if (stripeIndices[f] != 0 || fieldIndices[f] != 0)
					...
					for (Halfedge he: f.adjacentHalfedges()) {
						...
						if (polylineIndices[he.edge()].size() == 0) {
							...
							edgeIndices[idx].insert(edgeIndices[idx].end(), polylineIndices[he.edge()].rbegin(), polylineIndices[he.edge()].rend());
						}
						++idx;
					}
			...
			std::vector<std::vector<size_t>> removeCrossings(std::vector<std::pair<double, std::array<size_t, 2>>>& values, int N,
				...
				for (int i = 0; i < 3; ++i)
					removed[i] = std::vector<bool>(faceIndices[i].size(), false);
				...
				for (size_t i = 0; i < 3; ++i)
					for (size_t j = 0; j < faceIndices[i].size(); ++j)
						if (!removed[i][j])
							crossingIndices[i].push_back(faceIndices[i][j]);

				return crossingIndices;
			}

			// finds the minimum of abs(dot(v - w, vector)) for all points w except those on ignoredEdge
			double findMin(const std::vector<std::vector<Vector3>>& points, const Vector3& v, const Vector3& vector, size_t ignoredEdge) {
			...
			void connectIsolinesOnSingularities(EmbeddedGeometryInterface& geometry, const CornerData<double>& stripeValues,
												...
												EdgeData<std::vector<size_t>>& polylineIndices, const FaceData<Vector3>& vectorField,
												std::vector<Vector3>& vertices, std::vector<std::array<size_t, 2>>& edges) {
				SurfaceMesh& mesh = geometry.mesh;
				...
				for (Face f: mesh.faces()) {
					if (stripeIndices[f] != 0) {
						...
						for (Halfedge he: f.adjacentHalfedges()) {
							...
							if (he.next() == f.halfedge())
								isoPoints =
										crossingsModulo2Pi(stripeValues[he.corner()], stripeValues[he.next().corner()] + 2 * stripeIndices[f] * PI);
							else
		...
		std::tuple<std::vector<Vector3>, std::vector<std::array<size_t, 2>>> extractPolylinesFromStripePattern(
				EmbeddedGeometryInterface& geometry, const CornerData<double>& values, const FaceData<int>& stripeIndices,
				const FaceData<int>& fieldIndices, const VertexData<Vector2>& directionField, bool connectOnSingularities) {
			SurfaceMesh& mesh = geometry.mesh;

			std::vector<Vector3> points;
			std::vector<std::array<size_t, 2>> edges;
			EdgeData<std::vector<size_t>> indicesPerEdge;
			std::tie(points, edges, indicesPerEdge) = extractCrossingsFromStripePattern(geometry, values, stripeIndices, fieldIndices);
			...
			if (connectOnSingularities) {
				...
				for (Face f: mesh.faces()) {
					...
					for (Vertex v: f.adjacentVertices()) {
						...
						avgDir += (geometry.vertexTangentBasis[v][0] * vector.x + geometry.vertexTangentBasis[v][1] * vector.y).normalize();
					}

					vectorField[f] = avgDir.normalize();
				}

				connectIsolinesOnSingularities(geometry, values, stripeIndices, fieldIndices, indicesPerEdge, vectorField, points, edges);
			}
		...
		std::tuple<std::vector<Vector3>, std::vector<std::array<size_t, 2>>> computeStripePatternPolylines(EmbeddedGeometryInterface& geometry,
																										   const VertexData<double>& frequencies,
																										   const VertexData<Vector2>& directionField,
																										   bool connectIsolines) {
			...
			return extractPolylinesFromStripePattern(geometry, stripeValues, stripeIndices, fieldIndices, directionField, connectIsolines);
		}
