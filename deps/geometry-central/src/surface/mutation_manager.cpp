...
namespace geometrycentral {
	namespace surface {
		...
		MutationManager::MutationManager(ManifoldSurfaceMesh& mesh_, VertexPositionGeometry& geometry_) : mesh(mesh_), geometry(&geometry_) {
		...
		void MutationManager::setConstraintCallbacks() {
			...
			auto updateConstraintsAfterEdgeSplit = [&](Halfedge newHe1, Halfedge newHe2, double tSplit, std::array<bool, 3> oldConstraints) {
		...
		void MutationManager::clearRepositionableVertices() {
		...
		void MutationManager::clearSplittableEdges() {
			splittableEdges = EdgeData<bool>();
		}
		bool MutationManager::maySplitEdge(Edge e) const {
			return splittableEdges.size() == 0 || splittableEdges[e];
		}
		...
		void MutationManager::clearCollapsibleEdges() {
			collapsibleEdges = EdgeData<bool>();
		}
		bool MutationManager::mayCollapseEdge(Edge e) const {
			if (collapsibleEdges.size() > 0 && !collapsibleEdges[e])
				return false;
			if (repositionableVertices.size() > 0) {
				for (Vertex v: e.adjacentVertices()) {
					if (!repositionableVertices[v])
						return false;
				}
			}
		...
		void MutationManager::clearFlippableEdges() {
			flippableEdges = EdgeData<bool>();
		}
		bool MutationManager::mayFlipEdge(Edge e) const {
			return flippableEdges.size() == 0 || flippableEdges[e];
		}
		...
		void MutationManager::clearSplittableFaces() {
			splittableFaces = FaceData<bool>();
		}
		bool MutationManager::maySplitFace(Face f) const {
			return splittableFaces.size() == 0 || splittableFaces[f];
		}
		...
		*/
		...
		MutationPolicyHandle::MutationPolicyHandle(MutationManager& manager_, MutationPolicy* policy_) : manager(manager_), policy(policy_) {}
		void MutationPolicyHandle::remove() {
