	...
	namespace surface {
		...
		std::string SimplePolygonMesh::detectFileType(std::string filename) {
			...
			if (std::find(std::begin(supportedMeshTypes), std::end(supportedMeshTypes), type) == std::end(supportedMeshTypes)) {
				throw std::runtime_error("Detected file type " + type + " to load mesh from " + filename + ". This is not a supported file type.");
			}
		...
		void SimplePolygonMesh::readMeshFromAsciiStlFile(std::istream& in) {
			...
			auto assertToken = [&](const std::string& expected) {
				...
				if (token != expected) {
					...
					errorMessage << "Failed to parse ASCII stl file." << std::endl
								 << "Error on line " << lineNum << ". Expected \"" << expected << "\" but token \"" << token << "\"" << std::endl
			...
			while (nextLine() && !startsWithToken(line, "endsolid")) {
				...
				Vector3 faceNormal =
						cross(vertexCoordinates[face[1]] - vertexCoordinates[face[0]], vertexCoordinates[face[2]] - vertexCoordinates[face[0]]);
		...
		void SimplePolygonMesh::readMeshFromOffFile(std::istream& in) {
			...
			if (headerLine.rfind("OFF", 0) != 0)
				throw std::runtime_error("does not seem to be valid OFF file");
		...
		std::vector<size_t> SimplePolygonMesh::stripUnusedVertices() {
			...
			for (auto poly: polygons) {
				for (auto i: poly) {
					GC_SAFETY_ASSERT(i < nV, "polygon list has index " + std::to_string(i) + " >= num vertices " + std::to_string(nV));
					vertexUsed[i] = true;
				}
			}
		...
		void SimplePolygonMesh::stripFacesWithDuplicateVertices() {
			stripInvalidFaceWorker(false, false, true);
		}

		void SimplePolygonMesh::stripInvalidFaces() {
			stripInvalidFaceWorker(true, true, true);
		}

		void SimplePolygonMesh::stripInvalidFaceWorker(bool removeWithBadInds, bool removeLowDegree, bool removeWithRepeatedInds) {
