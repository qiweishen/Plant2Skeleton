...
namespace geometrycentral {
	namespace surface {
		...
		Vector3 boundaryVertexTangent(VertexPositionGeometry& geom, Vertex v, MutationManager& mm) {
			if (v.isBoundary()) {
				auto edgeVec = [&](Edge e) -> Vector3 {
					return (geom.vertexPositions[e.halfedge().tipVertex()] - geom.vertexPositions[e.halfedge().tailVertex()]).normalize();
				};
		...
		inline Vector3 projectToLine(Vector3 v, Vector3 tangent) {
		...
		double findMeanTargetL(SurfaceMesh& mesh, VertexPositionGeometry& geom, Edge e, double flatLength, double curvatureAdaptation) {
			double averageH = 0;
		...
		size_t fixDelaunay(ManifoldSurfaceMesh& mesh, VertexPositionGeometry& geom, MutationManager& mm) {
			...
			while (!edgesToCheck.empty() && nFlips < flipMax) {
				...
				if (e.isBoundary() || isDelaunay(geom, e))
					continue;
		...
		double smoothByLaplacian(ManifoldSurfaceMesh& mesh, VertexPositionGeometry& geom, double stepSize, RemeshBoundaryCondition bc) {
			MutationManager mm(mesh, geom);
		...
		double smoothByCircumcenter(ManifoldSurfaceMesh& mesh, VertexPositionGeometry& geom, double stepSize, RemeshBoundaryCondition bc) {
			MutationManager mm(mesh, geom);
			return smoothByCircumcenter(mesh, geom, mm, stepSize, bc);
		}

		double smoothByCircumcenter(ManifoldSurfaceMesh& mesh, VertexPositionGeometry& geom, MutationManager& mm, double stepSize,
									RemeshBoundaryCondition bc) {
		...
		bool adjustEdgeLengths(ManifoldSurfaceMesh& mesh, VertexPositionGeometry& geom, MutationManager& mm, RemeshOptions options) {
			...
			while (!toSplit.empty()) {
				...
				double threshold = (useCurvatureAdaptation) ? findMeanTargetL(mesh, geom, e, flatLength, curvatureAdaptation) : flatLength;
