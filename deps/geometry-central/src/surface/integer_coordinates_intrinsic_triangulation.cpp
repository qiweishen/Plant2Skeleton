...
namespace geometrycentral {
	namespace surface {
		...
		namespace {
			...
			inline Vertex src(Edge e) {
				return e.halfedge().vertex();
			}
			inline Vertex dst(Edge e) {
				return e.halfedge().next().vertex();
			}
		...
		IntegerCoordinatesIntrinsicTriangulation::IntegerCoordinatesIntrinsicTriangulation(ManifoldSurfaceMesh& mesh_,
		...
		EdgeData<std::vector<SurfacePoint>> IntegerCoordinatesIntrinsicTriangulation::traceAllIntrinsicEdgesAlongInput() {
			...
			for (Edge eB: intrinsicMesh->edges()) {
				for (CommonSubdivisionPoint* pt: cs.pointsAlongB[eB]) {
					if (pt->intersectionType != CSIntersectionType::EDGE_PARALLEL)
						tracedEdges[eB].push_back(pt->posA);
				}
			}
		...
		std::vector<SurfacePoint> IntegerCoordinatesIntrinsicTriangulation::traceIntrinsicHalfedgeAlongInput(Halfedge intrinsicHe) {
			CommonSubdivision& cs = getCommonSubdivision();
			...
			if (intrinsicHe == intrinsicHe.edge().halfedge()) {
				...
				for (auto rit = pts.rbegin(); rit != pts.rend(); ++rit) {  // loop backwards
					if ((*rit)->intersectionType != CSIntersectionType::EDGE_PARALLEL)
						trajectory.push_back((*rit)->posA);
				}
			}
		...
		EdgeData<std::vector<SurfacePoint>> IntegerCoordinatesIntrinsicTriangulation::traceAllInputEdgesAlongIntrinsic() {
			...
			for (Edge eA: inputMesh.edges()) {
				for (CommonSubdivisionPoint* pt: cs.pointsAlongA[eA]) {
					if (pt->intersectionType != CSIntersectionType::EDGE_PARALLEL)
						tracedEdges[eA].push_back(pt->posB);
				}
			}
			return tracedEdges;
		}
			...
			if (commonSubdivision) {
				...
				for (size_t iC = 0; iC < ncCurves.components.size(); iC++) {
					...
					if (path.size() == 1 && std::get<0>(path[0]) < 0) {	 // shared edge
						...
						std::vector<std::pair<SurfacePoint, double>> geodesicPath = generateFullSingleGeodesicGeometry(*intrinsicMesh, *this, curve);
		...
		bool IntegerCoordinatesIntrinsicTriangulation::checkEdgeOriginal(Edge e) const {
			return normalCoordinates[e] == -1;
		}
		...
		void IntegerCoordinatesIntrinsicTriangulation::constructCommonSubdivision() {
			...
			for (size_t iV = 0; iV < intrinsicMesh->nVertices(); iV++) {
				...
				switch (pA.type) {
					case SurfacePointType::Vertex: {
						...
						cs.subdivisionPoints.push_back(CommonSubdivisionPoint{ CSIntersectionType::VERTEX_VERTEX, pA, SurfacePoint(vB), true });

						aVtx[vA] = &cs.subdivisionPoints[cs.subdivisionPoints.size() - 1];
						break;
					}
					case SurfacePointType::Edge:
						cs.subdivisionPoints.push_back(CommonSubdivisionPoint{ CSIntersectionType::EDGE_VERTEX, pA, SurfacePoint(vB), true });
						break;
					case SurfacePointType::Face:
						cs.subdivisionPoints.push_back(CommonSubdivisionPoint{ CSIntersectionType::FACE_VERTEX, pA, SurfacePoint(vB), true });
						break;
				}
			...
			for (Edge eA: inputMesh.edges()) {
				...
				for (size_t iC = 0; iC < compoundPath.components.size(); iC++) {
					...
					if (path.size() == 1 && std::get<0>(path[0]) < 0) {
						...
						if (first)
							cs.pointsAlongA[eA].push_back(aVtx[src(eA)]);
						...
						for (size_t iC = 0; iC < path.size(); ++iC) {
							...
							cs.subdivisionPoints.push_back(
									CommonSubdivisionPoint{ CSIntersectionType::EDGE_TRANSVERSE, SurfacePoint(eA, tA), ptB, positiveOrientation });
			...
			for (Edge eB: intrinsicMesh->edges()) {
				for (const auto& p: cs.pointsAlongB[eB]) {
					if (!p) {
						std::cout << "Missed a crossing on edge " << eB << " which is supposed to have " << normalCoordinates[eB] << std::endl;
				...
				for (size_t iC = 1; iC + 1 < cs.pointsAlongB[eB].size(); ++iC) {
					if (cs.pointsAlongB[eB][iC]->intersectionType == CSIntersectionType::VERTEX_VERTEX) {
						std::cout << "Error at crossing " << iC << " of " << cs.pointsAlongB[eB].size() << " on edge " << eB << std::endl;
						for (const auto& pt: cs.pointsAlongB[eB])
							std::cout << "\t" << *pt << std::endl;
						throw std::runtime_error(
								"encountered vertex intersection "
		...
		bool IntegerCoordinatesIntrinsicTriangulation::flipEdgeIfNotDelaunay(Edge e) {
			...
			std::array<Vertex, 4> incidentVertices{ e.halfedge().vertex(), e.halfedge().next().vertex(), e.halfedge().next().next().vertex(),
		...
		Halfedge IntegerCoordinatesIntrinsicTriangulation::getSharedInputEdge(Halfedge he) const {
			...
			while (iE < 0)
				iE += normalCoordinates.roundaboutDegrees[he.vertex()];
			...
			for (int i = 0; i < iE; i++)
				inputHe = inputHe.next().next().twin();

			return inputHe;
		}

		std::pair<SurfacePoint, std::array<int, 3>> IntegerCoordinatesIntrinsicTriangulation::computeFaceSplitData(Face f, Vector3 bary,
																												   bool verbose) {
			Face insertionFace;
			...
			if (normalCoordinates[f.halfedge().edge()] < 0 && normalCoordinates[f.halfedge().next().edge()] < 0 &&
				...
				// live inserted in
				std::array<Corner, 3> faceCorners{ f.halfedge().corner(), f.halfedge().next().corner(), f.halfedge().next().next().corner() };
				...
				for (size_t iC = 0; iC < 3 && !foundRegion; iC++) {
					...
					if (cornerCoord < 1 || counts[iC] > cornerCoord)
						continue;
		...
		std::pair<SurfacePoint, size_t> IntegerCoordinatesIntrinsicTriangulation::computeEdgeSplitData(Halfedge he, double tBary) {
			if (normalCoordinates[he.edge()] > 0) {
				...
				return std::make_pair(SurfacePoint(inputFace, (1 - tBarySegment) * segmentTailCoords + tBarySegment * segmentTipCoords),
									  crossingSegment);
		...
		Halfedge IntegerCoordinatesIntrinsicTriangulation::splitBoundaryEdge(Halfedge he, double bary, bool verbose) {
			...
			if (normalCoordinates[e] >= 0) {
				if (verbose)
					std::cout << "Easy Edge Split" << std::endl;
				...
				*/
				...
				if (verbose)
					std::cout << "Shared Edge Split" << std::endl;
				...
				GC_SAFETY_ASSERT(newHalfedge.isInterior() && !newHalfedge.twin().isInterior(), "I'm wrong about orientation conventions");
		...
		Halfedge IntegerCoordinatesIntrinsicTriangulation::splitInteriorEdge(Halfedge he, double bary, bool verbose) {
			...
			if (verbose)
				std::cout << "Splitting halfedge " << he << " | " << he.tailVertex() << " -> " << he.tipVertex() << "  at  " << bary << std::endl;
			...
			if (verbose)
				std::cout << "  inputPoint: " << inputPoint << "\tiSeg: " << iSeg << std::endl;
			...
			newEdgeLengths[2] = displacementLength(heBary(e.halfedge(), bary) - heBary(e.halfedge().next(), 1), faceEdgeLengths(e.halfedge().face()));
			...
			if (verbose) {
				...
				for (Face f: newVertex.adjacentFaces())
					std::cout << "\t" << f << " " << faceString(f) << std::endl;
			}
		...
		NormalCoordinatesCompoundCurve IntegerCoordinatesIntrinsicTriangulation::traceInputHalfedge(Halfedge inputHe, bool verbose) const {
			...
			do {
				...
				for (size_t iH = 0; iH < width; ++iH) {
					...
					if (verbose) {
						...
						std::cout << "\t iH = " << iH << " of " << width << " = " << startInd << " + " << em << " + " << endInd << std::endl;
			...
			do {
				...
				size_t em =
						positivePart(normalCoordinates[he.next().edge()] - normalCoordinates[he.edge()] - normalCoordinates[he.next().next().edge()]);
				size_t startInd = (normalCoordinates[he.edge()] == 0) ? 1 : 0;
				size_t endInd = (normalCoordinates[he.next().next().edge()] == 0) ? 1 : 0;

				size_t width = em + startInd + endInd;
				std::cerr << "iStart: " << iStart << "\tem: " << em << "\tstartInd: " << startInd << "\t endInd: " << endInd << std::endl;
				for (size_t iH = 0; iH < width; ++iH) {
		...
		std::pair<bool, NormalCoordinatesCurve> IntegerCoordinatesIntrinsicTriangulation::traceNextCurve(const NormalCoordinatesCurve& oldCurve,
																										 bool verbose) const {
			GC_SAFETY_ASSERT(!oldCurve.crossings.empty(),
		...
		Halfedge IntegerCoordinatesIntrinsicTriangulation::identifyInputEdge(const NormalCoordinatesCurve& path, bool verbose) const {
		...
		Face IntegerCoordinatesIntrinsicTriangulation::getParentFace(Face f) const {
				...
				if (vP.type == SurfacePointType::Face) {
					...
					for (Vertex w: f.adjacentVertices()) {
						if (!compatible(vertexLocations[w], parentFace)) {
							std::cout << "Big problem: " << v << " lies at " << vP << ", but " << w << " is located at " << vertexLocations[w]
									  << " which is not compatible" << std::endl;
							return Face();
						}
					}
			...
			for (Face f: u.adjacentFaces()) {
				if (containsVertex(f, v) && containsVertex(f, w))
					return f;
			}
		...
		FaceData<Vector2> interpolateTangentVectorsB(const IntegerCoordinatesIntrinsicTriangulation& tri, const CommonSubdivision& cs,
