...
namespace geometrycentral {
	namespace surface {
		...
		double Interval::stop() const {
			return m_next ? m_next->start() : m_edge_length;
		}

		double Interval::hypotenuse(double a, double b) const {
			return sqrt(a * a + b * b);
		}
		...
		void Interval::initialize(IntrinsicGeometryInterface& geom, Edge edge, double edge_length, SurfacePoint* source, unsigned source_index) {
			m_next = nullptr;
		...
		void IntervalList::find_closest_point(IntrinsicGeometryInterface& geom, const SurfacePoint point, double& offset, double& distance,
											  const_interval_pointer& interval, bool verbose) const {
			const_interval_pointer p = m_first;
		...
		interval_pointer& IntervalList::first() {
			return m_first;
		};
		Edge& IntervalList::edge() {
			return m_edge;
		};

		unsigned SurfacePointWithIndex::index() const {
			return m_index;
		}
		...
		namespace exactgeodesic {
			double compute_surface_distance(IntrinsicGeometryInterface& geom, const SurfacePoint& p1, const SurfacePoint& p2) {
				...
				GC_SAFETY_ASSERT(fShared != Face(),
								 "compute_surface_distance() err: Point " + std::to_string(p1) + " not adjacent to " + std::to_string(p2));
				...
				Vector3 triangleLengths{ geom.edgeLengths[fShared.halfedge().next().edge()],
										 geom.edgeLengths[fShared.halfedge().next().next().edge()], geom.edgeLengths[fShared.halfedge().edge()] };
			...
			std::pair<double, double> compute_local_coordinates(IntrinsicGeometryInterface& geom, Edge e, const SurfacePoint& point) {
				SurfacePoint eDummyPt(e, 0.5);
				Face fShared = sharedFace(point, eDummyPt);

				GC_SAFETY_ASSERT(fShared != Face(),
								 "compute_local_coordinates() err: Point " + std::to_string(point) + " not adjacent to " + std::to_string(e));
