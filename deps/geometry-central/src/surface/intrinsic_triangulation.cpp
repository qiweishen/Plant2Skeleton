...
namespace geometrycentral {
	namespace surface {
		...
		IntrinsicTriangulation::IntrinsicTriangulation(ManifoldSurfaceMesh& mesh_, IntrinsicGeometryInterface& inputGeom_) :
			EdgeLengthGeometry(*mesh_.copy().release()),
			inputMesh(mesh_),
			inputGeom(inputGeom_),
			intrinsicMesh(dynamic_cast<ManifoldSurfaceMesh*>(&mesh)) {
			if (!inputMesh.isCompressed()) {
				throw std::runtime_error("mesh should be compressed before constructing an intrinsic triangulation. Call mesh.compress().");
			}
		...
		void IntrinsicTriangulation::clearMarkedEdges() {
			markedEdges = EdgeData<bool>();
		}
		...
		double IntrinsicTriangulation::minAngleDegreesAtValidFaces(double minAngleSum) const {
			...
			for (Face f: intrinsicMesh->faces()) {
				if (!faceHasLargeAngleSums(f))
					continue;
				if (!parentFaceHasLargeAngleSums(f))
					continue;
		...
		std::vector<double> IntrinsicTriangulation::recoverTraceTValues(const std::vector<SurfacePoint>& edgeTrace) {
			...
			for (size_t iP = 0; iP + 1 < edgeTrace.size(); iP++) {
				...
				Vector3 triangleLengths{ inputGeom.edgeLengths[f.halfedge().next().edge()], inputGeom.edgeLengths[f.halfedge().next().next().edge()],
		...
		void IntrinsicTriangulation::delaunayRefine(double angleThreshDegrees, double circumradiusThresh, size_t maxInsertions) {
		...
		void IntrinsicTriangulation::triangulationChanged() {
