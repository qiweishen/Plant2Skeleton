...
namespace geometrycentral {
	namespace surface {
		...
		std::tuple<std::unique_ptr<ManifoldSurfaceMesh>, std::unique_ptr<VertexPositionGeometry>> loadMesh(std::string filename, std::string type) {
			return readManifoldSurfaceMesh(filename, type);
		}


		// Load a general surface mesh, which might or might not be manifold
		std::tuple<std::unique_ptr<SurfaceMesh>, std::unique_ptr<VertexPositionGeometry>> readSurfaceMesh(std::string filename, std::string type) {
			std::string loadType;
		...
		std::tuple<std::unique_ptr<SurfaceMesh>, std::unique_ptr<VertexPositionGeometry>> readSurfaceMesh(std::istream& in, std::string type) {
		...
		std::tuple<std::unique_ptr<ManifoldSurfaceMesh>, std::unique_ptr<VertexPositionGeometry>> readManifoldSurfaceMesh(std::string filename,
																														  std::string type) {
			std::string loadType;
			SimplePolygonMesh simpleMesh;
			simpleMesh.readMeshFromFile(filename, type, loadType);
		...
		std::tuple<std::unique_ptr<ManifoldSurfaceMesh>, std::unique_ptr<VertexPositionGeometry>> readManifoldSurfaceMesh(std::istream& in,
																														  std::string type) {
			std::string loadType = type;
			SimplePolygonMesh simpleMesh;
			simpleMesh.readMeshFromFile(in, type);
		...
		readParameterizedManifoldSurfaceMesh(std::string filename, std::string type) {
			...
			return makeManifoldSurfaceMeshAndGeometry(simpleMesh.polygons, {}, simpleMesh.vertexCoordinates, simpleMesh.paramCoordinates);
		}
		...
		void writeSurfaceMesh(SurfaceMesh& mesh, EmbeddedGeometryInterface& geometry, CornerData<Vector2>& texCoords, std::string filename,
							  std::string type) {
			SimplePolygonMesh simpleMesh(mesh.getFaceVertexList(), geometryToStdVector(mesh, geometry), paramToStdVector(mesh, texCoords));
			simpleMesh.writeMesh(filename, type);
		}
		...
		void writeSurfaceMesh(SurfaceMesh& mesh, EmbeddedGeometryInterface& geometry, CornerData<Vector2>& texCoords, std::ostream& out,
							  std::string type) {
			SimplePolygonMesh simpleMesh(mesh.getFaceVertexList(), geometryToStdVector(mesh, geometry), paramToStdVector(mesh, texCoords));
			simpleMesh.writeMesh(out, type);
		}
		...
		void WavefrontOBJ::writeTexCoords(std::ofstream& out, EmbeddedGeometryInterface& geometry, CornerData<Vector2>& texcoords) {
			SurfaceMesh& mesh(geometry.mesh);
		...
		void WavefrontOBJ::writeFaces(std::ofstream& out, EmbeddedGeometryInterface& geometry, bool useTexCoords, bool useNormals) {
			SurfaceMesh& mesh(geometry.mesh);
