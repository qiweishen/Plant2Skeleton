...
namespace geometrycentral {
	namespace surface {
		...
		void FlipEdgePath::replacePathSegment(SegmentID nextID, SegmentAngleType angleType, const std::vector<Halfedge>& newHalfedges) {
		...
		size_t FlipEdgePath::size() {
			return pathHeInfo.size();
		}
		...
		std::vector<Halfedge> FlipEdgePath::getHalfedgeList() {
			...
			while (walkID != INVALID_IND) {
				...
				walkID = prevID;
				if (walkID == firstID)
					break;								 // test at end so we don't insta-out
			}
		...
		FlipEdgeNetwork::FlipEdgeNetwork(ManifoldSurfaceMesh& mesh_, IntrinsicGeometryInterface& inputGeom,
			...
			mesh(*(tri->intrinsicMesh)),
			pathsAtEdge(mesh),
			isMarkedVertex(mesh, false) {
		...
		std::unique_ptr<FlipEdgeNetwork> FlipEdgeNetwork::constructFromDijkstraPath(ManifoldSurfaceMesh& mesh_, IntrinsicGeometryInterface& geom,
		...
		std::unique_ptr<FlipEdgeNetwork> FlipEdgeNetwork::constructFromPiecewiseDijkstraPath(ManifoldSurfaceMesh& mesh_,
																							 ...
																							 std::vector<Vertex> points, bool closed,
																							 bool markInterior) {
		...
		std::unique_ptr<FlipEdgeNetwork> FlipEdgeNetwork::constructFromEdgeSet(ManifoldSurfaceMesh& mesh_, IntrinsicGeometryInterface& geom,
			...
			for (Halfedge heStart: mesh.halfedges()) {
				...
				if (!inPath[heStart.edge()] || walked[heStart.edge()] || !isEndpoint[heStart.vertex()])
					continue;
				...
				while (true) {
					...
					if (isEndpoint[heCurr.twin().vertex()])
						break;
			...
			for (Halfedge heStart: mesh.halfedges()) {
				// Check if we should start a walk
				if (!inPath[heStart.edge()] || walked[heStart.edge()])
					continue;
		...
		double FlipEdgeNetwork::minAngleIsotopy() {
			...
			for (auto& epPtr: paths) {
				...
				for (auto it: path.pathHeInfo) {
					...
					if (!straightenAroundMarkedVertices && isMarkedVertex[currHe.twin().vertex()])
						continue;
		...
		std::tuple<SegmentAngleType, double> FlipEdgeNetwork::locallyShortestTestWithType(Halfedge hePrev, Halfedge heNext) {
			if (hePrev == Halfedge())
				return std::make_tuple(SegmentAngleType::Shortest, std::numeric_limits<double>::infinity());
		...
		FlipEdgeNetwork::ShortestReturnBoth FlipEdgeNetwork::locallyShortestTestWithBoth(Halfedge hePrev, Halfedge heNext) {
			ShortestReturnBoth result{ SegmentAngleType::Shortest, std::numeric_limits<double>::infinity(), SegmentAngleType::Shortest,
		...
		bool FlipEdgeNetwork::wedgeIsClearEndpointsOnly(const FlipPathSegment& pathSegmentNext, SegmentAngleType type) {
			...
			switch (type) {
				...
				case SegmentAngleType::LeftTurn: {
					// Check bounding edges
					if (getOutsideSegment(hePrev) != pathSegmentPrev && getOutsideSegment(hePrev).isEndpoint())
						return false;
					if (getOutsideSegment(heNext) != pathSegmentNext && getOutsideSegment(heNext).isEndpoint())
						return false;
		...
		void FlipEdgeNetwork::addToWedgeAngleQueue(const FlipPathSegment& pathSegment) {
			...
			if (testResult.minType == SegmentAngleType::Shortest)
				return;	 // If smaller is straight, nothing to do
			wedgeAngleQueue.emplace(testResult.minAngle, testResult.minType, pathSegment);

			// Check the larger of the two angles
			if (testResult.maxType == SegmentAngleType::Shortest)
				return;
		...
		void FlipEdgeNetwork::delaunayRefine(double areaThresh, size_t maxInsertions, double angleBound) {
			...
			auto updatePathOnSplit = [&](Edge oldE, Halfedge newHe1, Halfedge newHe2) { updatePathAfterEdgeSplit(oldE.halfedge(), newHe1); };
		...
		void FlipEdgeNetwork::bezierSubdivideRecursive(size_t nRoundsRemaining, const Vertex firstControlCall, const Vertex lastControlCall) {
			...
			auto findSegmentAfter = [&](Vertex v) {
				...
				for (Halfedge he: v.outgoingHalfedges()) {
					if (!pathsAtEdge[he.edge()].empty()) {
						...
						if (s.halfedge() == he) {
							if (toReturn.id != INVALID_IND)
								throw std::runtime_error("multiple paths at vertex in bezier");
							toReturn = s;
						}
					}
				}
			...
			while (true) {
					...
					for (auto& segRegion: regions) {
						...
						while (true) {
							if (currP != firstSeg) {
								...
								if (s.id != INVALID_IND)
									addToWedgeAngleQueue(s);  // might need to straighten since we unmarked
							}
				...
				for (auto& segRegion: regions) {
					...
					while (true) {
						...
						if (currP == lastP)
							throw std::runtime_error("couldn't find split segment");
		...
		bool FlipEdgeNetwork::edgeInPath(Edge e) {
			return !pathsAtEdge[e].empty();
		}

		bool FlipEdgeNetwork::halfedgeInPath(Halfedge he) {
			return !pathsAtEdge[he.edge()].empty();
		}
		...
		FlipPathSegment FlipEdgeNetwork::getFirst() {
			for (auto& epPtr: paths) {
				...
				for (auto it: path.pathHeInfo) {
					...
					if (prevID == INVALID_IND)
						return FlipPathSegment{ epPtr.get(), currID };
				}
			}
		...
		void FlipEdgeNetwork::validateHalfedgesOnly() {
			for (auto& epPtr: paths) {
				...
				for (auto it: path.pathHeInfo) {
					...
					if (currHe.getMesh() == nullptr)
						throw std::runtime_error("bad halfedge entry");
				}
			}
		}
		...
		void FlipEdgeNetwork::validate() {
			...
			for (auto& epPtr: paths) {
				...
				for (auto it: path.pathHeInfo) {
					...
					if (prevID == INVALID_IND) {
						if (path.isClosed || prevInvalidCount > 0)
							throw std::runtime_error("too many invalid prev IDs");
						if (!isMarkedVertex[currHe.vertex()])
							throw std::runtime_error("path begin endpoint isn't a marked vertex");
						prevInvalidCount++;
					}
					if (nextID == INVALID_IND) {
						if (path.isClosed || nextInvalidCount > 0)
							throw std::runtime_error("too many invalid next IDs");
						if (!isMarkedVertex[currHe.twin().vertex()])
				...
				if (!path.isClosed) {
					if (prevInvalidCount != 1)
						throw std::runtime_error("open path does not have first halfedge");
					if (nextInvalidCount != 1)
						throw std::runtime_error("open path does not have last halfedge");
				}
			}
