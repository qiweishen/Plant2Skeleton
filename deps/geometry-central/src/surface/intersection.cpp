...
namespace geometrycentral {
	namespace surface {
		...
		SurfaceIntersectionResult intersections(EmbeddedGeometryInterface& geometry1, EmbeddedGeometryInterface& geometry2, bool selfCheck) {
			...
			for (Face f: mesh1.faces()) {
				...
				for (Face g: mesh2.faces()) {
					...
					if (selfCheck && g.getIndex() >= f.getIndex())
						continue;
					...
					if (!(u[0] == v[0] || u[0] == v[1] || u[0] == v[2] || u[1] == v[0] || u[1] == v[1] || u[1] == v[2] || u[2] == v[0] ||
						  u[2] == v[1] || u[2] == v[2])) {
