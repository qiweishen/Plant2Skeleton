...
namespace geometrycentral {
	namespace surface {
		...
		HeatMethodDistanceSolver::HeatMethodDistanceSolver(IntrinsicGeometryInterface& geom_, double tCoef_, bool useRobustLaplacian_) :
		...
		SurfaceMesh& HeatMethodDistanceSolver::getMesh() {
			return useRobustLaplacian ? *tuftedMesh : mesh;
		}
		...
		VertexData<double> HeatMethodDistanceSolver::computeDistance(const std::vector<SurfacePoint>& sourcePoints) {
			...
			auto baryDist = [&](Vector3 b1, Vector3 b2, const std::array<double, 3>& edgeLengths) {
				...
				if (!(d2 <= 0))
					d2 = 0.;  // ensure it's negative so the sqrt below succeed
				return std::sqrt(-d2);
			};
