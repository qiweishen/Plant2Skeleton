...
namespace geometrycentral {
	namespace surface {
		...
		RichSurfaceMeshData::RichSurfaceMeshData(SurfaceMesh& mesh_, std::string filename_) : plyData(filename_), mesh(&mesh_) {}
		...
		RichSurfaceMeshData::RichSurfaceMeshData(std::istream& in_) : plyData(in_) {
			loadMeshFromFile();
		}
		RichSurfaceMeshData::RichSurfaceMeshData(std::string filename_) : plyData(filename_) {
			loadMeshFromFile();
		}

		void RichSurfaceMeshData::loadMeshFromFile() {
			if (mesh != nullptr)
				throw std::runtime_error("cannot load mesh multiple times");

			if (!plyData.hasElement("gc_internal_halfedge")) {
				throw std::runtime_error("Cannot load mesh from file, file was saved without connectiviy. Call addMeshConnectivity() before saving.");
			}
  ...
  if(!useImplicitTwin) {
    ...
    eHalfedgeArr    = fromSmallerVec(plyData.getElement("gc_internal_edge").getProperty<uint32_t>("gc_internal_eHalfedgeArr"));
			...
			if (useImplicitTwin) {
				...
				mesh = new SurfaceMesh(heNextArr, heVertexArr, heFaceArr, vHalfedgeArr, fHalfedgeArr, heSiblingArr, heEdgeArr, heOrientArr,
									   eHalfedgeArr, fHalfedgeArrB.size());
			}
		}

		std::tuple<std::unique_ptr<SurfaceMesh>, std::unique_ptr<RichSurfaceMeshData>> RichSurfaceMeshData::readMeshAndData(std::string filename) {
		...
		std::tuple<std::unique_ptr<SurfaceMesh>, std::unique_ptr<RichSurfaceMeshData>> RichSurfaceMeshData::readMeshAndData(std::istream& in) {
			std::unique_ptr<RichSurfaceMeshData> data(new RichSurfaceMeshData(in));
			return std::make_tuple(std::unique_ptr<SurfaceMesh>(data->mesh), std::move(data));
		}
		std::tuple<std::unique_ptr<ManifoldSurfaceMesh>, std::unique_ptr<RichSurfaceMeshData>> RichSurfaceMeshData::readManifoldMeshAndData(
				std::string filename) {
			std::unique_ptr<RichSurfaceMeshData> data(new RichSurfaceMeshData(filename));
		...
		VertexData<Vector3> RichSurfaceMeshData::getVertexColors() {
			...
			try {
				...
				try {
					...
					throw std::runtime_error("Could not find vertex colors in Binary file, as uchar or float");
				}
			}
		}

		void RichSurfaceMeshData::write(std::string filename) {
			plyData.write(filename, outputFormat);
		}
		void RichSurfaceMeshData::write(std::ostream& out) {
			plyData.write(out, outputFormat);
		}
