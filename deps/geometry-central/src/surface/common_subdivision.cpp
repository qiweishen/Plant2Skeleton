	...
	namespace surface {
		...
		namespace {
			...
			std::vector<T> toStdVector(MeshData<E, T> data) {
				...
				for (size_t i = 0; i < (size_t) dataVals.size(); ++i)
					vec.push_back(dataVals(i));
				return vec;
			}
		...
		std::ostream& operator<<(std::ostream& out, const CommonSubdivisionPoint& pt) {
			out << "CommonSubdivisionPoint{ intersectionType: " << pt.intersectionType << "posA: " << pt.posA << ", posB: " << pt.posB
				<< ", orientation: " << pt.orientation << "}";
			return out;
		}

		CommonSubdivision::CommonSubdivision(ManifoldSurfaceMesh& meshA_, ManifoldSurfaceMesh& meshB_) : meshA(meshA_), meshB(meshB_) {
		...
		EdgeData<double> CommonSubdivision::interpolateEdgeLengthsA(const EdgeData<double>& lengthA) {
			...
			auto fEdgeLengths = [&](Face f) -> Vector3 {  // Gather face's edge lengths
				return Vector3{ lengthA[f.halfedge().next().edge()], lengthA[f.halfedge().next().next().edge()], lengthA[f.halfedge().edge()] };
			};
		...
		EdgeData<double> CommonSubdivision::interpolateEdgeLengthsB(const EdgeData<double>& lengthB) {
			...
			auto fEdgeLengths = [&](Face f) -> Vector3 {  // Gather face's edge lengths
				return Vector3{ lengthB[f.halfedge().next().edge()], lengthB[f.halfedge().next().next().edge()], lengthB[f.halfedge().edge()] };
			};
		...
		size_t CommonSubdivision::nVertices() const {
			...
			for (Edge eB: meshB.edges())
				nV += intersectionsB(eB);
			return nV;
		}

		size_t CommonSubdivision::nEdges() const {
			return std::get<1>(elementCounts());
		}

		size_t CommonSubdivision::nFaces() const {
			return std::get<2>(elementCounts());
		}
		...
		void CommonSubdivision::constructMeshData(std::vector<std::vector<size_t>>& faces_out, std::vector<CommonSubdivisionPoint*>& parents_out,
				...
				for (size_t iC = 1; iC + 1 < pointsAlongB[eB].size(); ++iC) {
					...
					if (pointsAlongB[eB][iC]->intersectionType == CSIntersectionType::VERTEX_VERTEX) {
						throw std::runtime_error(
								"encountered vertex intersection in the middle of an "
								"edge");
					}
				}
			...
			for (Face f: meshB.faces()) {
				...
				if (ij != ij.edge().halfedge())
					std::reverse(std::begin(pij), std::end(pij));
				std::vector<size_t> pjk = crossingVtxIds[jk.edge()];
				if (jk != jk.edge().halfedge())
					std::reverse(std::begin(pjk), std::end(pjk));
				std::vector<size_t> pki = crossingVtxIds[ki.edge()];
				if (ki != ki.edge().halfedge())
					std::reverse(std::begin(pki), std::end(pki));
		...
		std::vector<std::vector<size_t>> sliceFace(const std::vector<size_t>& pij, const std::vector<size_t>& pjk, const std::vector<size_t>& pki) {
		...
		std::vector<std::vector<size_t>> sliceNicelyOrderedFace(const std::vector<size_t>& pij, const std::vector<size_t>& pjk,
			...
			if (nij > njk + nki) {
				...
				GC_SAFETY_ASSERT((nij + njk + nki) % 2 == 0,
								 "normal coordinates which obey the triangle "
		...
		void CommonSubdivision::writeToFile(std::string filename, const VertexData<Vector3>& vertexPositions, int kColors) {
			...
			for (Corner c: mesh->corners())
				texCoords[c] = Vector2{ faceColors[sourceFaceB[c.face()]], 0.5 };
			...
			for (Face f: mesh.faces()) {
				...
				for (int iCandColor = 0; iCandColor < kColors; iCandColor++) {
					...
					for (Face fN: f.adjacentFaces()) {
						// Single neighbors
						if (faceColor[fN] == kVals[iCandColor])
							adjacencyScore = 0;

						// Double neighbors
						for (Face fNN: fN.adjacentFaces()) {
							if (faceColor[fNN] == kVals[iCandColor])
								adjacencyScore = std::min(adjacencyScore, 1);
						}
					}

					// Accept if less adjacent then previous best adjacent score
					if (adjacencyScore > bestAdjacencyScore || (adjacencyScore == bestAdjacencyScore && lastUsedT[iCandColor] < oldestColorT)) {
				...
				if (bestColor == -1)
					throw std::runtime_error("failed to color");
