...
namespace geometrycentral {
	namespace surface {
		...
		VertexData<double> SignedHeatSolver::computeDistance(const std::vector<Curve>& curves, const std::vector<SurfacePoint>& points,
		...
		VertexData<double> SignedHeatSolver::computeDistance(const std::vector<Curve>& curves, const SignedHeatOptions& options) {
			// call generic version
			return computeDistance(curves, std::vector<SurfacePoint>(), options);
		}

		VertexData<double> SignedHeatSolver::computeDistance(const std::vector<SurfacePoint>& points, const SignedHeatOptions& options) {
			// call generic version
		...
		Vector<std::complex<double>> SignedHeatSolver::integrateVectorHeatFlow(const std::vector<Curve>& curves,
			...
			for (const SurfacePoint& point: points)
				buildUnsignedPointSource(point, X0);
			...
			if (!options.preserveSourceNormals) {
				...
				for (size_t i = 0; i < E; i++)
					Xt[i] = std::complex<double>(X[i], X[E + i]);
			}
		...
		VertexData<double> SignedHeatSolver::integrateVectorField(const Vector<std::complex<double>>& Xt, const std::vector<Curve>& curves,
		...
		void SignedHeatSolver::buildUnsignedCurveSource(const Curve& curve, Vector<std::complex<double>>& X0) {
			...
			std::vector<double> endLengths = {
				...
				geom.edgeLengths[determineHalfedgeFromVertices(curve.nodes[nNodes - 2].vertex, curve.nodes[nNodes - 1].vertex).edge()]
			};
		...
		void SignedHeatSolver::buildUnsignedVertexSource(const Vertex& v, Vector<std::complex<double>>& X0, double weight) const {
			double angleSum = 0.;
			for (Corner c: v.adjacentCorners())
		...
		Vector<double> SignedHeatSolver::integrateWithZeroSetConstraint(const Vector<double>& rhs, const std::vector<Curve>& curves,
																		const std::vector<SurfacePoint>& points, const SignedHeatOptions& options) {
		...
		Vector<double> SignedHeatSolver::integrateWithLevelSetConstraints(const Vector<double>& rhs, const std::vector<Curve>& curves,
																		  const SignedHeatOptions& options) {
			geom.requireCotanLaplacian();
		 ...
		 */
		std::complex<double> SignedHeatSolver::projectedNormal(const SurfacePoint& pA, const SurfacePoint& pB, const Edge& e) const {
			BarycentricVector segment(pA, pB);
			BarycentricVector segNormal = segment.rotate90(geom);
		...
		double SignedHeatSolver::computeAverageValueOnSource(const Vector<double>& phi, const std::vector<Curve>& curves) const {
		...
		void SignedHeatSolver::ensureHaveVectorHeatSolver() {
			if (vectorHeatSolver != nullptr && !timeUpdated)
				return;
			timeUpdated = false;
		...
		void SignedHeatSolver::ensureHaveHalfedgeVectorsInVertex() {
			if (halfedgeVectorsInVertex.size() > 0)
				return;
			...
			for (Vertex v: mesh.vertices()) {
				...
				do {
					halfedgeVectorsInVertex[currHe] = std::complex<double>(Vector2::fromAngle(coordSum)) * geom.edgeLengths[currHe.edge()];
		...
		double SignedHeatSolver::scalarCrouzeixRaviart(const SurfacePoint& p, const Edge& e) const {
			...
			switch (p.type) {
				case (SurfacePointType::Vertex): {
					...
					if (sharedFace == Face())
						return 0.;
					if (p.vertex == e.firstVertex() || p.vertex == e.secondVertex())
						return 1.;
					return -1;
					break;
				}
				case (SurfacePointType::Edge): {
					...
					for (Halfedge he: sharedFace.adjacentHalfedges()) {
						if (he.next().edge() == e)
							return 1. - 2. * pInFace.faceCoords[idx];
						idx++;
					}
					break;
				}
