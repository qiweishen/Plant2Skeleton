...
namespace geometrycentral {
	namespace surface {
		...
		VertexData<Vector2> VectorHeatMethodSolver::transportTangentVectors(const std::vector<std::tuple<Vertex, Vector2>>& sources) {
		...
		VertexData<Vector2> VectorHeatMethodSolver::transportTangentVectors(const std::vector<std::tuple<SurfacePoint, Vector2>>& sources) {
			if (sources.size() == 0) {
		...
		void VectorHeatMethodSolver::addVertexOutwardBall(Vertex vert, Vector<std::complex<double>>& distGradRHS) {
			...
			for (Halfedge he: vert.outgoingHalfedges()) {
				...
				if (he.isInterior()) {
					...
					Vector2 valInEdgeBasis{ -theta * std::sin(theta) / (2.0 * h), (theta * std::cos(theta) - std::sin(theta)) / (2.0 * h) };
				...
				if (he.isInterior()) {
					...
					Vector2 valInEdgeBasis{ -(theta * std::cos(alpha) + std::cos(alpha - theta) * std::sin(theta)) / (2.0 * h),
											-(std::cos(alpha) - std::cos(alpha - 2 * theta) + 2 * theta * std::sin(alpha)) / (4.0 * h) };
