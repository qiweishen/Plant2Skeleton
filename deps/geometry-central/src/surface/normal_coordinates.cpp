...
namespace geometrycentral {
	namespace surface {
		...
		int NormalCoordinates::operator[](Edge e) const {
			return edgeCoords[e];
		}
		int NormalCoordinates::operator[](Corner c) const {
			return cornerCoord(c);
		}
		int& NormalCoordinates::operator[](Edge e) {
			return edgeCoords[e];
		}
		...
		std::array<int, 3> NormalCoordinates::computeVertexInsertionDataGeodesic(IntrinsicGeometryInterface& geo, Face f, Vector3 location) {
		...
		void NormalCoordinates::applyVertexInsertionData(Vertex newVertex, const std::array<int, 3>& update) {
			size_t iE = 0;
			GC_SAFETY_ASSERT(newVertex.degree() == 3,
							 "Inserting a vertex into a triangle should produce a degree "
							 "3 vertex, but " +
									 std::to_string(newVertex) + " has degree " + std::to_string(newVertex.degree()));
		...
		std::array<int, 4> NormalCoordinates::computeInteriorEdgeSplitDataGeodesic(IntrinsicGeometryInterface& geo, Edge e, double location) {
			/*       Flip rotates edge clockwise
			 *          k
			 *        / |\
			 *       /    \
		...
		std::array<int, 4> NormalCoordinates::computeInteriorEdgeSplitDataCombinatorial(IntrinsicGeometryInterface& geo, Edge e, size_t iSeg) {
			if (edgeCoords[e] > 0) {
		...
		std::array<int, 3> NormalCoordinates::computeBoundaryEdgeSplitDataGeodesic(IntrinsicGeometryInterface& geo, Edge e, double location) {
			GC_SAFETY_ASSERT(e.isBoundary(),
							 "can only call computeBoundaryEdgeSplitDataGeodesic on a "
							 "boundary edge");
		...
		std::tuple<NormalCoordinatesCurve, int> NormalCoordinates::topologicalTraceBidirectional(Halfedge he, int index) const {
			...
			if (forwardTrace.crossings.size() > 1 && forwardTrace.crossings.front().second == forwardTrace.crossings.back().second) {
		...
		std::vector<NormalCoordinatesCurve> NormalCoordinates::topologicalTraceAllCurves() const {
			...
			for (Edge e: mesh.edges())
				visited[e] = std::vector<char>(positivePart(edgeCoords[e]), false);
			...
			for (const NormalCoordinatesCurve& curve: curves) {
				if (std::get<0>(curve.crossings[0]) >= 0) {
					...
					if (curve.crossings.size() == 1) {
						...
						GC_SAFETY_ASSERT(index >= 0,
										 "Only paths along a single mesh edge are "
					...
					GC_SAFETY_ASSERT(curve.crossings.size() == 1,
									 "Only paths along a single mesh edge are allowed to "
		...
		//                            Geodesic Routines
		//==============================================================================
		std::vector<std::vector<SurfacePoint>> NormalCoordinates::generateGeodesicGeometry(IntrinsicGeometryInterface& geo) const {
			std::vector<NormalCoordinatesCurve> curves = topologicalTraceAllCurves();
		...
		double NormalCoordinates::generateGeodesicCrossingLocation(IntrinsicGeometryInterface& geo, Halfedge he, int ind) const {
		...
		std::vector<double> NormalCoordinates::generateGeodesicCrossingLocations(IntrinsicGeometryInterface& geo, Halfedge he) const {
		...
		int flipNormalCoordinates(int nij, int njk, int nki, int nil, int nlj) {
			...
			int doubledAnswer = 2 * Clji + 2 * Ckij + abs(Cjil - Cjki) + abs(Cilj - Cijk) - Elji - Ekij + 2 * Eilj + 2 * Eijk + 2 * Ejil + 2 * Ejki;
		...
		// variable names;
		std::pair<size_t, size_t> flipRoundabouts(int nij, int njk, int nki, int nil, int nlj, int nkl, size_t rki, size_t rlj, size_t dk,
												  size_t dl) {
		...
		size_t strictCornerCoord(int nij, int njk, int nki) {
			return positivePart(cornerCoord(nij, njk, nki));
		}

		// Given trace info (like the output from topotrace, generate geodesic
		// geometry)
		std::vector<std::vector<SurfacePoint>> generateGeodesicGeometry(ManifoldSurfaceMesh& mesh, IntrinsicGeometryInterface& geo,
		...
		std::vector<std::pair<SurfacePoint, double>> generateFullSingleGeodesicGeometry(ManifoldSurfaceMesh& mesh, IntrinsicGeometryInterface& geo,
																						const NormalCoordinatesCurve& curve, bool verbose) {
			...
			auto layOutNextFaceFromAngle = [&](Halfedge he, Halfedge prevHe, const std::array<Vector2, 3>& prevPositions, double theta) {
		...
		std::array<int, 3> computeVertexInsertionCrossingCounts(Vector3 bary, const std::array<std::vector<double>, 3>& boundaryCrossings) {
