...
namespace geometrycentral {
	namespace surface {
		...
		SurfaceMesh::SurfaceMesh(const std::vector<size_t>& heNextArr_, const std::vector<size_t>& heVertexArr_,
			...
			useImplicitTwinFlag(false),
			heSiblingArr(heSiblingArr_),
			heEdgeArr(heEdgeArr_),
			heOrientArr(heOrientArr_),
			...
			for (Vertex v: vertices()) {
				{  // Manually traverse the incoming halfedges incident on this vertex
					...
					for (size_t traverseInd = rangeStart; traverseInd < rangeEnd; traverseInd++) {
						...
						size_t iHeB = vertexIterationCacheHeIndexIn[(((traverseInd - rangeStart) + 1) % (rangeEnd - rangeStart)) + rangeStart];
				...
				{  // Manually traverse the outgoing halfedges incident on this vertex
					...
					for (size_t traverseInd = rangeStart; traverseInd < rangeEnd; traverseInd++) {
						...
						size_t iHeB = vertexIterationCacheHeIndexOut[(((traverseInd - rangeStart) + 1) % (rangeEnd - rangeStart)) + rangeStart];

						if (heVertexArr[iHeA] != v.getIndex())
							throw std::runtime_error("out A problem");
						if (heVertexArr[iHeB] != v.getIndex())
							throw std::runtime_error("out B problem");
		...
		void SurfaceMesh::printStatistics() const {
			...
			std::cout << "    # halfedges =  " << nHalfedges() << "  (" << nInteriorHalfedges() << " interior, " << nExteriorHalfedges()
					  << " exterior)" << std::endl;
		...
		std::unique_ptr<ManifoldSurfaceMesh> SurfaceMesh::toManifoldMesh() {
			if (!isManifold())
				throw std::runtime_error("must be manifold to create manifold surface mesh");
			if (!isOriented())
				throw std::runtime_error("must be oriented to create manifold surface mesh");
		...
		void SurfaceMesh::generateVertexIterationCache(std::vector<size_t>& vertexIterationCacheHeIndex,
													   std::vector<size_t>& vertexIterationCacheVertexStart, bool incoming, bool skipDead) {
		...
		bool SurfaceMesh::flip(Edge eFlip, bool preventSelfEdges) {
			...
			if (ha2 == hb1 || hb2 == ha1)
				return false;  // incident on degree 1 vertex
			...
			if (va.halfedge() == ha1)
				vHalfedgeArr[va.getIndex()] = hb2.getIndex();
			if (vb.halfedge() == hb1)
				vHalfedgeArr[vb.getIndex()] = ha2.getIndex();
		...
		Edge SurfaceMesh::separateToNewEdge(Halfedge heA, Halfedge heB) {
			if (usesImplicitTwin())
				throw std::runtime_error("Cannot separate edge from manifold mesh; all are already manifold. Try general SurfaceMesh.");

			if (heA.edge() != heB.edge())
				throw std::runtime_error("halfedges must be incident on same edge");
			if (heA == heB)
				throw std::runtime_error("halfedges must be distinct");
		...
		void SurfaceMesh::validateConnectivity() {
			...
			if (nHalfedgesCount > nHalfedgesFillCount)
				throw std::logic_error("halfedge count > halfedge fill");
			if (nHalfedgesFillCount > nHalfedgesCapacityCount)
				throw std::logic_error("halfedge fill > halfedge capacity");

			if (nVerticesCount > nVerticesFillCount)
				throw std::logic_error("vertex count > vertex fill");
			if (nVerticesFillCount > nVerticesCapacityCount)
				throw std::logic_error("vertex fill > vertex capacity");

			if (nFacesCount > nFacesFillCount)
				throw std::logic_error("face count > face fill");
			...
			if (nHalfedgesCount > std::numeric_limits<uint64_t>::max() / 2)
				throw std::logic_error("halfedge count overflow");
			if (nExteriorHalfedges() > std::numeric_limits<uint64_t>::max() / 2)
				throw std::logic_error("exterior halfedge count overflow");
			if (nVerticesCount > std::numeric_limits<uint64_t>::max() / 2)
				throw std::logic_error("vertex count overflow");
			if (nEdgesCount > std::numeric_limits<uint64_t>::max() / 2)
				throw std::logic_error("edge count overflow");
			if (nFacesCount > std::numeric_limits<uint64_t>::max() / 2)
				throw std::logic_error("face count overflow");

			// Helpers to check the validity of references
			auto validateVertex = [&](size_t iV, std::string msg) {
				if (iV >= nVerticesFillCount || vertexIsDead(iV))
					throw std::logic_error(msg + " - bad vertex reference");
			};
			auto validateHalfedge = [&](size_t iHe, std::string msg) {
				if (iHe >= nHalfedgesFillCount || halfedgeIsDead(iHe))
					throw std::logic_error(msg + " - bad halfedge reference");
			};
			auto validateEdge = [&](size_t iE, std::string msg) {
				if (iE >= nEdgesFillCount || edgeIsDead(iE))
					throw std::logic_error(msg + " - bad edge reference");
			};
			auto validateFace = [&](size_t iF, std::string msg) {
				if (iF >= nFacesCapacityCount || faceIsDead(iF) || (iF >= nFacesFillCount && iF < nFacesCapacityCount - nBoundaryLoopsCount)) {
			...
			for (Halfedge he: halfedges()) {
				...
				do {
					...
					if (count > nHalfedges())
						throw std::logic_error("(he sibling) halfedge sibling doesn't cycle back");
			...
			for (Face f: faces()) {
				if (f.halfedge().face() != f)
					throw std::logic_error("f.halfedge().face() is not f");
				...
				do {
					if (currHe.face() != f)
						throw std::logic_error("face.halfedge doesn't match halfedge.face");
					currHe = currHe.next();
					count++;
					if (count > nHalfedgesCount)
						throw std::logic_error("next forms non-face loop");
				} while (currHe != firstHe);

				if (count < 2)
					throw std::logic_error("face of degree < 2");
			}


			// Check face & next sanity
			for (BoundaryLoop b: boundaryLoops()) {
				if (b.asFace().asBoundaryLoop() != b)
					throw std::logic_error("b.asFace().asBoundaryLoop() is not fixed point");

				if (b.halfedge().face() != b.asFace())
					throw std::logic_error("bl.halfedge().face() is not bl");

				if (!b.halfedge().face().isBoundaryLoop())
					throw std::logic_error("bl.halfedge().face() is not a boundary loop");
				...
				do {
					...
					count++;
					if (count > nHalfedgesCount)
						throw std::logic_error("(boundary loop) next forms non-face loop");
				} while (currHe != firstHe);

				if (count < 2)
					throw std::logic_error("(boundary loop) face of degree < 2");
			}
			...
			for (Halfedge he: halfedges()) {
				...
				if (!he.isInterior()) {
					if (he == he.edge().halfedge())
						throw std::logic_error("exterior halfedge is e.halfedge()");
					if (!he.twin().isInterior())
						throw std::logic_error("he and he.twin() are both exterior");
				}
				...
				if (useImplicitTwinFlag) {
					...
					size_t count = 0;
					do {
						if (currHe.face() != he.face())
							throw std::logic_error("he.next.**.face doesn't match he.face");
			...
			for (Vertex v: vertices()) {
				...
				for (Halfedge currHe: v.outgoingHalfedges()) {
					if (count > nHalfedgesCount)
						throw std::logic_error("vertex outgoing halfedges has bad cycle");
					if (currHe.vertex() != v)
						throw std::logic_error("vertex.halfedge doesn't match halfedge.vertex");
					count++;
				}
			}
			...
			if (usesImplicitTwin()) {
				...
				if (!isManifold()) {
					throw std::logic_error(
							"Mesh with underlying type ManifoldSurfaceMesh is not actually manifold, likely due to a "
