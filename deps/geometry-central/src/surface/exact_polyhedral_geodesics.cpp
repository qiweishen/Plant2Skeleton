...
namespace geometrycentral {
	namespace surface {
		...
		namespace {
			...
			bool isCCW(Vector2 p1, Vector2 p2, double eps = 0.) {
				return cross(p1, p2) > -eps;
			}
			...
			public:
				...
				void considerEdgeForProcessing(UnfoldingEdge uEdge) {
					...
					if (!isStrictlyCCW(uEdge.rightPos, uEdge.leftLimit) || !isStrictlyCCW(uEdge.rightLimit, uEdge.leftPos))
						return;
				...
				void spawnWindowsFromSouce(Vertex source, double sourceDist) {
					for (Halfedge he: source.outgoingHalfedges()) {
						...
						if (!he.isInterior())
							continue;  // no opposite edge for boundary halfedges
				...
				void computeDistancesWithinRadius(Vertex centerVert, double distanceThreshParam) {
					...
					while (!edgesToProcess.empty()) {
						...
						Vector2 newPos = layoutTriangleVertex(uEdge.leftPos, uEdge.rightPos, geom.edgeLengths[uEdge.acrossHe.next().edge()],
      ...
      considerEdgeForProcessing(newWindowLeft);
		...
		std::unordered_map<Vertex, double> vertexGeodesicDistanceWithinRadius(IntrinsicGeometryInterface& geom, Vertex centerVert,
																			  double distanceThresh) {
