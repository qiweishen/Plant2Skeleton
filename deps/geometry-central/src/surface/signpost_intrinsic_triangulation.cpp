...
namespace geometrycentral {
	namespace surface {
		...
		std::vector<SurfacePoint> SignpostIntrinsicTriangulation::traceInputHalfedgeAlongIntrinsic(Halfedge inputHe, bool trimEnd) {
			// Optimization: don't bother tracing original edges, just report them directly
		...
		SurfacePoint SignpostIntrinsicTriangulation::equivalentPointOnIntrinsic(const SurfacePoint& pointOnInput) {
			...
			SurfacePoint traceVertexLoc = SurfacePoint(intrinsicMesh->vertex(traceVertex.getIndex()));	// (see note abouve about getIndex())
		...
		bool SignpostIntrinsicTriangulation::checkEdgeOriginal(Edge e) const {
			return edgeIsOriginal[e];
		}
		...
		void SignpostIntrinsicTriangulation::flipEdgeManual(Edge e, double newLength, double forwardAngle, double reverseAngle, bool isOrig,
															bool reverseFlip) {
			int flipCount = reverseFlip ? 3 : 1;  // three flips give opposite orientaiton
			for (int i = 0; i < flipCount; i++) {
				bool flipped = intrinsicMesh->flip(e, false);
				if (!flipped)
					throw std::runtime_error("could not perform manual flip");
			}
		...
		void SignpostIntrinsicTriangulation::constructCommonSubdivision() {
			...
			for (Edge e: inputMesh.edges()) {
				...
				std::sort(vec.begin(), vec.end(),
						  [](CommonSubdivisionPoint*& a, CommonSubdivisionPoint*& b) -> bool { return a->posA.tEdge < b->posA.tEdge; });
