	...
	namespace surface {
		...
		namespace {
			...
			inline std::array<Vector2, 3> vertexCoordinatesInTriangle(IntrinsicGeometryInterface& geom, Face face) {
				return { Vector2{ 0., 0. }, geom.halfedgeVectorsInFace[face.halfedge()], -geom.halfedgeVectorsInFace[face.halfedge().next().next()] };
			}
			...
			inline TraceSubResult traceInFaceBarycentric(IntrinsicGeometryInterface& geom, Face face, Vector3 startPoint, Vector3 vecBary,
														 Vector2 vecCartesianDir, double vecCartesianLen, std::array<bool, 3> edgeIsHittable,
				...
				if (TRACE_PRINT) {
					...
					cout << "  face: " << face << " startPoint " << startPoint << " vecBary = " << vecBary << " vecCartesian " << vecCartesianDir
						 << " " << vecCartesianLen << endl;
				}
				...
				*/
				...
				double tCross =
						endPointOnEdge[(iOppVertEnd + 2) % 3] / (endPointOnEdge[(iOppVertEnd + 1) % 3] + endPointOnEdge[(iOppVertEnd + 2) % 3]);
				if (TRACE_PRINT) {
			...
			inline TraceSubResult traceInFaceTowardsEdge(IntrinsicGeometryInterface& geom, Halfedge towardsHe, Vector2 vecCartesianDir,
														 double vecCartesianLen, const TraceOptions& traceOptions) {
				// Gather some values
				...
				return traceInFaceBarycentric(geom, face, startPoint, vecBaryCanonicalFixed, vecCartesianDir, vecCartesianLen, hittable,
											  traceOptions);
			}
			...
			inline TraceSubResult traceInFaceFromEdge(IntrinsicGeometryInterface& geom, Halfedge fromHe, double tCrossFrom,
													  Vector2 traceVecInHalfedgeDir, double traceVecInHalfedgeLen, const TraceOptions& traceOptions) {
				...
				*/
				...
				if (TRACE_PRINT)
					cout << "  face trace from edge " << fromHe << " vec = " << traceVecInHalfedgeDir << endl;
				...
				if (TRACE_PRINT)
					cout << "    vec in face before project " << traceVecInFaceHalfedgeDir << endl;
				traceVecInFaceHalfedgeDir.y = std::fmax(traceVecInFaceHalfedgeDir.y, TRACE_EPS_LOOSE);
				if (TRACE_PRINT)
					cout << "    vec in face after project " << traceVecInFaceHalfedgeDir << endl;
				...
				if (TRACE_PRINT)
					cout << "    vec bary after project " << vecBaryFromEdgeDir << endl;
				...
				return traceInFaceBarycentric(geom, face, startPoint, vecBaryCanonicalFixedDir * traceVecInHalfedgeLen, traceVecInFaceDir,
											  traceVecInHalfedgeLen, hittable, traceOptions);
			}


			// Trace starting from an edge
			inline TraceSubResult traceGeodesic_fromEdge(IntrinsicGeometryInterface& geom, Edge currEdge, double tEdge, Vector2 currVecDir,
														 double currVecLen, const TraceOptions& traceOptions) {
				if (TRACE_PRINT)
					cout << "  edge trace " << currEdge << " tEdge = " << tEdge << " edge vec = " << currVecDir << endl;
			...
			inline TraceSubResult traceGeodesic_fromFace(IntrinsicGeometryInterface& geom, Face currFace, Vector3 faceBary, Vector2 currVecDir,
														 double currVecLen, const TraceOptions& traceOptions) {
				...
				return traceInFaceBarycentric(geom, currFace, faceBary, vecBary, currVecDir, currVecLen, { true, true, true }, traceOptions);
			}


			// Trace starting from a vertex (with a rescaled cartesian vector)
			inline TraceSubResult traceGeodesic_fromVertex(IntrinsicGeometryInterface& geom, Vertex currVert, Vector2 currVecDir, double currVecLen,
														   const TraceOptions& traceOptions) {
				if (TRACE_PRINT)
					cout << "  vertex trace " << currVert << " edge vec = " << currVecDir << endl;
				...
				do {
					...
					if (crossStart > 0. && crossEnd <= 0.) {
						...
						if (TRACE_PRINT)
							cout << "    wedge match! relative angle " << traceDirRelativeToStart << endl;
						if (TRACE_PRINT)
							cout << "    cross start = " << crossStart << " cross end = " << crossEnd << endl;
						break;
					}
				...
				if (wedgeHe == Halfedge()) {
					if (TRACE_PRINT)
						cout << "  no wedge worked. following closest edge with dir " << minCrossHalfedgeDir << endl;
					// Convert to edge coordinates
					currVecDir = convertVecToEdge(minCrossHalfedge, minCrossHalfedgeDir);
					return traceGeodesic_fromEdge(geom, minCrossHalfedge.edge(), convertTToEdge(minCrossHalfedge, 0.), currVecDir, currVecLen,
												  traceOptions);
				}
			...
			void traceGeodesic_iterative(IntrinsicGeometryInterface& geom, TraceGeodesicResult& result, TraceSubResult prevTraceEnd,
				...
				while (!prevTraceEnd.terminated) {
					...
					prevTraceEnd = traceInFaceFromEdge(geom, prevTraceEnd.crossHe, prevTraceEnd.tCross, prevTraceEnd.traceVectorInHalfedgeDir,
		...
		TraceGeodesicResult traceGeodesic(IntrinsicGeometryInterface& geom, SurfacePoint startP, Vector2 traceVec, const TraceOptions& traceOptions) {
			...
			switch (startP.type) {
				...
				case SurfacePointType::Edge: {
					prevTraceEnd = traceGeodesic_fromEdge(geom, startP.edge, startP.tEdge, unit(traceVec), norm(traceVec), traceOptions);
					break;
				}
				case SurfacePointType::Face: {
					prevTraceEnd = traceGeodesic_fromFace(geom, startP.face, startP.faceCoords, unit(traceVec), norm(traceVec), traceOptions);
					break;
				}
			}
		...
		TraceGeodesicResult traceGeodesic(IntrinsicGeometryInterface& geom, Face startFace, Vector3 startBary, Vector3 traceBaryVec,
										  const TraceOptions& traceOptions) {
			geom.requireVertexAngleSums();
			geom.requireHalfedgeVectorsInVertex();
			geom.requireHalfedgeVectorsInFace();
			...
			if (TRACE_PRINT) {
				cout << "\n>>> Trace query (barycentric) from " << startFace << " " << startBary << " vec = " << traceBaryVec << endl;
			}
			...
			TraceSubResult prevTraceEnd = traceInFaceBarycentric(geom, startFace, startBary, traceBaryVec, unit(traceVectorCartesian),
		...
		bool trimTraceResult(TraceGeodesicResult& traceResult, Vertex targetVertex) {
			...
			// Check success
			if (traceResult.pathPoints.empty())
				return false;
			...
			switch (b.type) {
				...
				case SurfacePointType::Edge: {
					...
					if (bHe.vertex() == targetVertex)
						return true;
					if (bHe.twin().vertex() == targetVertex)
						return true;
					if (bHe.next().next().vertex() == targetVertex)
						return true;
					if (bHe.twin().next().next().vertex() == targetVertex)
						return true;
					return false;
					break;
				}
