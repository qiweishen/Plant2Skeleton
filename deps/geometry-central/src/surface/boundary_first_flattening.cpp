...
namespace geometrycentral {
	namespace surface {
		...
		BFF::BFF(ManifoldSurfaceMesh& mesh_, IntrinsicGeometryInterface& geo_) : mesh(mesh_), geo(geo_) {
			GC_SAFETY_ASSERT(mesh.eulerCharacteristic() == 2 && mesh.nBoundaryLoops() == 1, "Input to BFF must be a topological disk");
		...
		VertexData<Vector2> BFF::flattenFromExteriorAngles(const VertexData<double>& kData) {
			...
			GC_SAFETY_ASSERT(abs(kBdy.sum() - 2 * M_PI) < 1e-3,
							 "BFF error: target exterior angles must sum to 2 pi, but the input sums to " + std::to_string(kBdy.sum()));
		...
		std::array<Vector<double>, 2> BFF::computeBoundaryPositions(const Vector<double>& uBdy, const Vector<double>& kBdy) {
			...
			Vector<double> roundedLength = targetLength - Ninv * T.transpose() * (T * Ninv * T.transpose()).inverse() * (T * targetLength);
