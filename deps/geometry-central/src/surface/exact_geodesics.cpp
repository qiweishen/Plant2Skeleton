	...
	namespace surface {
		...
		void GeodesicAlgorithmExact::propagate(const std::vector<Vertex>& sources, double max_propagation_distance,
			...
			for (const Vertex v: sources)
				source_surface_points.push_back(SurfacePoint(v));
			for (const Vertex v: stop_points)
				stop_surface_points.push_back(SurfacePoint(v));
		...
		void GeodesicAlgorithmExact::propagate(const Vertex& source, double max_propagation_distance, const std::vector<Vertex>& stop_points) {
			// Call general version
		...
		void GeodesicAlgorithmExact::best_point_on_the_edge_set(const SurfacePoint& point, std::vector<Edge> const& storage,
																...
																double& best_interval_position, bool verbose) const {
				...
				case SurfacePointType::Vertex: {
					...
					for (Halfedge he: v.outgoingHalfedges()) {
						if (he.face().isBoundaryLoop())
							continue;  // don't try to trace along virtual boundary face
		...
		double GeodesicAlgorithmExact::compute_positive_intersection(double start, double pseudo_x, double pseudo_y, double sin_alpha,
																	 double cos_alpha) {
			assert(pseudo_y < 0);
		...
		void GeodesicAlgorithmExact::list_edges_visible_from_source(const SurfacePoint& source, std::vector<Edge>& storage) const {
			switch (source.type) {
				case SurfacePointType::Vertex: {
		...
		void GeodesicAlgorithmExact::propagate(const std::vector<SurfacePoint>& sources, double max_propagation_distance,
			...
			while (!m_queue.empty()) {
				...
				for (Halfedge neighboring_halfedge: edge.adjacentHalfedges()) {
					...
					if (!neighboring_halfedge.isInterior())
						continue;

					// just in case, always propagate boundary edges
					if (!edge.isBoundary() && min_interval->halfedge() == neighboring_halfedge)
						continue;
					...
					Halfedge next_halfedge = (neighboring_halfedge.orientation() == he.orientation()) ? neighboring_halfedge.next().next()
					...
					if (num_propagated) {
						...
						bool const invert =
								(next_halfedge == neighboring_halfedge.next().next()) ? next_halfedge.orientation() : !next_halfedge.orientation();
		...
		bool GeodesicAlgorithmExact::check_stop_conditions(unsigned& index) const {
			...
			while (index < m_stop_vertices.size()) {
				...
				double distance = v.halfedge().orientation() ? interval_list(edge)->signal(0.0) : interval_list(edge)->signal(geom.edgeLengths[edge]);
		...
		unsigned GeodesicAlgorithmExact::compute_propagated_parameters(double pseudo_x, double pseudo_y,
		...
		void GeodesicAlgorithmExact::construct_propagated_intervals(bool invert, Halfedge halfedge, IntervalWithStop* candidates,
		...
		double GeodesicAlgorithmExact::getDistance(const SurfacePoint& point) const {
			return closestSource(point).second;
		}
		...
		Vector2 GeodesicAlgorithmExact::getDistanceGradient(const SurfacePoint& point) const {
			...
			Vector2 faceDisplacement = barycentricDisplacementToCartesian(vertexCoordinatesInTriangle(geom, fShared), baryPoint - baryClosestPoint);
		...
		const_interval_pointer GeodesicAlgorithmExact::best_first_interval(const SurfacePoint& point, double& best_total_distance,
		...
		IntervalList GeodesicAlgorithmExact::getEdgeIntervals(Edge e) const {
			return m_edge_interval_lists[e];
		}
