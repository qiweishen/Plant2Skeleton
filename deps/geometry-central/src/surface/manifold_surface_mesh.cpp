		...
		ManifoldSurfaceMesh::ManifoldSurfaceMesh(const std::vector<std::vector<size_t>>& polygons) : SurfaceMesh(true) {
			...
			for (size_t iFace = 0; iFace < nFacesCount; iFace++) {
				...
				for (size_t iFaceHe = 0; iFaceHe < faceDegree; iFaceHe++) {
					...
					// Some sanity checks
					GC_SAFETY_ASSERT(indTail != indTip, "self-edge in face list " + std::to_string(indTail) + " -- " + std::to_string(indTip));
			...
			// boundary walk below.
			{
				...
				for (size_t iHe = 0; iHe < nHalfedgesCount; iHe++) {
					if (heNextArr[iHe] == INVALID_IND) {
						...
						GC_SAFETY_ASSERT(!vertexOnBoundary[v], "vertex " + std::to_string(v) + " appears in more than one boundary loop");
...
// boundary walk below.
		...
		bool ManifoldSurfaceMesh::isOriented() {
			return true;
		}

		VertexData<bool> ManifoldSurfaceMesh::getVertexManifoldStatus() {
			return VertexData<bool>(*this, true);
		}
		EdgeData<bool> ManifoldSurfaceMesh::getEdgeManifoldStatus() {
			return EdgeData<bool>(*this, true);
		}
		EdgeData<bool> ManifoldSurfaceMesh::getEdgeOrientedStatus() {
			return EdgeData<bool>(*this, true);
		}
		...
		Halfedge ManifoldSurfaceMesh::splitEdgeTriangular(Edge e) {
			...
			GC_SAFETY_ASSERT(e.isBoundary() || e.halfedge().twin().face().isTriangle(), "splitEdgeTriangular requires triangular faces");
		...
		*/
		...
		Face ManifoldSurfaceMesh::removeVertex(Vertex v) {
			...
			for (Halfedge he: v.outgoingHalfedges()) {
				if (!checkAndAddUniqueEdge(he.edge()))
					return Face();	// check that the ring is distinct/safe
			...
			for (Halfedge outgoingHe: v.outgoingHalfedges()) {	// this orbits cw
				...
				while (true) {
					if (!checkAndAddUniqueEdge(ringHe.edge()))
						return Face();	// check that the ring is distinct/safe
					if (!checkAndAddUniqueVertex(ringHe.tailVertex()))
						return Face();
		...
		*/
		...
		bool ManifoldSurfaceMesh::hasBoundary() {
			return nBoundaryLoopsCount > 0;
		}
