...
// Suitesparse version
		...
		if (underdetermined) {
			internals->factorization = SuiteSparseQR_factorize<typename SOLVER_ENTRYTYPE<T>::type>(ordering, internals->zero_tolerance,
																								   internals->cMatTrans, internals->context);
		} else {
			internals->factorization = SuiteSparseQR_factorize<typename SOLVER_ENTRYTYPE<T>::type>(ordering, internals->zero_tolerance,
																								   internals->cMat, internals->context);
		}
...
// Suitesparse version
		...
		if (underdetermined) {
			// solve y = R^-T b
			cholmod_dense* y =
					SuiteSparseQR_solve<typename SOLVER_ENTRYTYPE<T>::type>(SPQR_RTX_EQUALS_ETB, internals->factorization, inVec, internals->context);

			// compute x = Q*y
			outVec = SuiteSparseQR_qmult<typename SOLVER_ENTRYTYPE<T>::type>(SPQR_QX, internals->factorization, y, internals->context);
			cholmod_l_free_dense(&y, internals->context);

		} else {
			// compute y = Q^T b
			cholmod_dense* y = SuiteSparseQR_qmult<typename SOLVER_ENTRYTYPE<T>::type>(SPQR_QTX, internals->factorization, inVec, internals->context);

			// solve x = R^-1 y
			outVec = SuiteSparseQR_solve<typename SOLVER_ENTRYTYPE<T>::type>(SPQR_RETX_EQUALS_B, internals->factorization, y, internals->context);
...
// Eigen version
	...
	template Vector<std::complex<double>> solve(SparseMatrix<std::complex<double>>& A, const Vector<std::complex<double>>& rhs);
