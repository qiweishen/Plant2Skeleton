...
namespace geometrycentral {
	...
	namespace {
		...
		void umfSolve<std::complex<double>>(size_t N, cholmod_sparse* mat, void* numericFac, Vector<std::complex<double>>& x,
			...
			SuiteSparse_long* cMat_i = (SuiteSparse_long*) mat->i;
			double* cMat_x = (double*) mat->x;
			umfpack_zl_solve(UMFPACK_A, cMat_p, cMat_i, cMat_x, NULL, (double*) &(x[0]), NULL, (double*) &(rhs[0]), NULL, numericFac, NULL, NULL);
		}
...
// Eigen variant
	...
	template Vector<std::complex<double>> solveSquare(SparseMatrix<std::complex<double>>& A, const Vector<std::complex<double>>& rhs);
