...
namespace geometrycentral {
	...
	template<typename T>
	Vector<T> smallestEigenvectorPositiveDefinite(SparseMatrix<T>& energyMatrix, SparseMatrix<T>& massMatrix, size_t nIterations) {
	...
	template<typename T>
	std::vector<Vector<T>> smallestKEigenvectorsPositiveDefinite(SparseMatrix<T>& energyMatrix, SparseMatrix<T>& massMatrix, size_t kEigenvalues,
																 size_t nIterations) {
	...
	template<typename T>
	std::vector<Vector<T>> smallestKEigenvectorsPositiveDefiniteTol(SparseMatrix<T>& energyMatrix, SparseMatrix<T>& massMatrix, size_t kEigenvalues,
																	double tol) {
		std::vector<Vector<T>> res;
	...
	template Vector<double> smallestEigenvectorPositiveDefinite(SparseMatrix<double>& energyMatrix, SparseMatrix<double>& massMatrix,
																size_t nIterations);
	template Vector<float> smallestEigenvectorPositiveDefinite(SparseMatrix<float>& energyMatrix, SparseMatrix<float>& massMatrix,
															   size_t nIterations);
	template Vector<std::complex<double>> smallestEigenvectorPositiveDefinite(SparseMatrix<std::complex<double>>& energyMatrix,
																			  SparseMatrix<std::complex<double>>& massMatrix, size_t nIterations);

	template std::vector<Vector<float>> smallestKEigenvectorsPositiveDefinite(SparseMatrix<float>& energyMatrix, SparseMatrix<float>& massMatrix,
																			  size_t kEigenvalues, size_t nIterations);
	template std::vector<Vector<double>> smallestKEigenvectorsPositiveDefinite(SparseMatrix<double>& energyMatrix, SparseMatrix<double>& massMatrix,
																			   size_t kEigenvalues, size_t nIterations);
	template std::vector<Vector<std::complex<double>>> smallestKEigenvectorsPositiveDefinite(SparseMatrix<std::complex<double>>& energyMatrix,
																							 SparseMatrix<std::complex<double>>& massMatrix,
																							 size_t kEigenvalues, size_t nIterations);


	template std::vector<Vector<float>> smallestKEigenvectorsPositiveDefiniteTol(SparseMatrix<float>& energyMatrix, SparseMatrix<float>& massMatrix,
	...
	template std::vector<Vector<double>> smallestKEigenvectorsPositiveDefiniteTol(SparseMatrix<double>& energyMatrix,
																				  SparseMatrix<double>& massMatrix, size_t kEigenvalues, double tol);
	template std::vector<Vector<std::complex<double>>> smallestKEigenvectorsPositiveDefiniteTol(SparseMatrix<std::complex<double>>& energyMatrix,
																								SparseMatrix<std::complex<double>>& massMatrix,
																								size_t kEigenvalues, double tol);

	template Vector<double> smallestEigenvectorSquare(SparseMatrix<double>& energyMatrix, SparseMatrix<double>& massMatrix, size_t nIterations);
	template Vector<float> smallestEigenvectorSquare(SparseMatrix<float>& energyMatrix, SparseMatrix<float>& massMatrix, size_t nIterations);
	template Vector<std::complex<double>> smallestEigenvectorSquare(SparseMatrix<std::complex<double>>& energyMatrix,
																	SparseMatrix<std::complex<double>>& massMatrix, size_t nIterations);

	template Vector<double> largestEigenvector(SparseMatrix<double>& energyMatrix, SparseMatrix<double>& massMatrix, size_t nIterations);
	template Vector<float> largestEigenvector(SparseMatrix<float>& energyMatrix, SparseMatrix<float>& massMatrix, size_t nIterations);
