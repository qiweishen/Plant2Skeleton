...
namespace geometrycentral {
	...
	template<typename T>
	PositiveDefiniteSolver<T>::PositiveDefiniteSolver(SparseMatrix<T>& mat) : LinearSolver<T>(mat), internals(new PSDSolverInternals<T>()) {
		// Check some sanity
	...
	template Vector<std::complex<double>> solvePositiveDefinite<std::complex<double>>(SparseMatrix<std::complex<double>>& A,
