...
namespace geometrycentral {
	...
	public:
		// == Types
		typedef nanoflann::KDTreeSingleIndexAdaptor<nanoflann::L2_Simple_Adaptor<double, Vector3Adaptor>, Vector3Adaptor, 3> KD_Tree_T;
		...
		std::vector<size_t> kNearest(Vector3 query, size_t k) {
			if (k > data.rawPoints.size())
				throw std::runtime_error("k is greater than number of points");
		...
		std::vector<size_t> kNearestNeighbors(size_t sourceInd, size_t k) {
			if ((k + 1) > data.rawPoints.size())
				throw std::runtime_error("k+1 is greater than number of points");
	...
	NearestNeighborFinder::NearestNeighborFinder(const std::vector<Vector3>& points) {
		impl.reset(new KNNImpl(points));
	}
	NearestNeighborFinder::~NearestNeighborFinder() = default;

	std::vector<size_t> NearestNeighborFinder::kNearest(Vector3 query, size_t k) {
		return impl->kNearest(query, k);
	}
