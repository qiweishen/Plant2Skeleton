...
#pragma omp critical
				...
				while (anyChanged) {
					...
					for (size_t iMiddle = 0; iMiddle < N; iMiddle++) {
						if (sortInds[iMiddle] == INVALID_IND)
						...
						if (isBoundary(prev, curr) || isBoundary(curr, next))
							continue;
		...
		PointData<std::vector<std::array<size_t, 3>>> handleToInds(PointCloud& cloud,
		...
		std::vector<std::vector<size_t>> handleToFlatInds(PointCloud& cloud, const PointData<std::vector<std::array<Point, 3>>>& localResult) {
			GC_SAFETY_ASSERT(cloud.isCompressed(), "cloud must be compressed");
