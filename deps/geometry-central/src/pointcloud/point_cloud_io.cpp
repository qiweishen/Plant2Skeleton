	...
	namespace pointcloud {
		...
		namespace {
			...
			std::string typeFromFilename(std::string filename) {
				...
				if (std::find(std::begin(supportedPointCloudTypes), std::end(supportedPointCloudTypes), type) == std::end(supportedPointCloudTypes)) {
		...
		std::tuple<std::unique_ptr<PointCloud>, std::unique_ptr<PointPositionGeometry>> readPointCloud(std::string filename, std::string type) {
		...
		std::tuple<std::unique_ptr<PointCloud>, std::unique_ptr<PointPositionGeometry>> readPointCloud(std::istream& in, std::string type) {
			if (type == "obj") {
			...
			return std::tuple<std::unique_ptr<PointCloud>, std::unique_ptr<PointPositionGeometry>>{ nullptr, nullptr };	 // unreachable
		}
