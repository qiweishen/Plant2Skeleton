...
namespace geometrycentral {
	namespace pointcloud {
		...
		void PointCloudHeatSolver::ensureHaveVectorHeatSolver() {
			if (vectorHeatSolver != nullptr)
				return;

			geom.requireConnectionLaplacian();
			geom.tuftedGeom->requireVertexLumpedMassMatrix();

			heatDistanceWorker.reset(new surface::HeatMethodDistanceSolver(*geom.tuftedGeom, tCoef, false));  // we already have the tufted IDT
		...
		PointData<Vector2> PointCloudHeatSolver::transportTangentVectors(const std::vector<std::tuple<Point, Vector2>>& sources) {
			GC_SAFETY_ASSERT(sources.size() != 0, "must have at least one source");
		...
		PointData<double> PointCloudHeatSolver::computeSignedDistance(const std::vector<std::vector<Point>>& curves,
																	  const PointData<Vector3>& cloudNormals, const SignedHeatOptions& options) {
			...
			for (const auto& curve: curves) {
				...
				for (size_t i = 0; i < nNodes - 1; i++) {
					...
					Vector2 curveNormalA = { dot(geom.tangentBasis[vA][0], inPlaneNormalA), dot(geom.tangentBasis[vA][1], inPlaneNormalA) };
					Vector2 curveNormalB = { dot(geom.tangentBasis[vB][0], inPlaneNormalB), dot(geom.tangentBasis[vB][1], inPlaneNormalB) };
