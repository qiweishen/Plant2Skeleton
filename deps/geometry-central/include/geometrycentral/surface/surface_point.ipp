...
namespace geometrycentral {
	namespace surface {
		...
		inline SurfacePoint::SurfacePoint(Face f, Vector3 faceCoords_) : type(SurfacePointType::Face), face(f), faceCoords(faceCoords_) {}
		...
		inline SurfacePoint SurfacePoint::inFace(Face targetFace) const {
			...
			throw std::logic_error("SurfacePoint " + std::to_string(*this) + " not adjacent to target face " + std::to_string(targetFace));
			return *this;
		}
		...
		inline SurfacePoint SurfacePoint::inEdge(Edge targetEdge) const {
			...
			throw std::logic_error("SurfacePoint " + std::to_string(*this) + " not adjacent to target edge " + std::to_string(targetEdge));
			return *this;
		}
		...
		inline void SurfacePoint::validate() const {
			...
			switch (type) {
				case SurfacePointType::Vertex: {
					if (vertex == Vertex())
						throw std::logic_error("surface point with Type::Vertex has invalid vertex ref");
					break;
				}
				case SurfacePointType::Edge: {
					if (edge == Edge())
						throw std::logic_error("surface point with Type::Edge has invalid edge ref");
					if (!std::isfinite(tEdge))
						throw std::logic_error("surface point with Type::Edge has non-finite tEdge");
				...
				case SurfacePointType::Face: {
					if (face == Face())
						throw std::logic_error("surface point with Type::Face has invalid face ref");
					if (!isfinite(faceCoords))
						throw std::logic_error("surface point with Type::Face has non-finite face coords");
					...
					if ((faceCoords.x + faceCoords.y + faceCoords.z) > (1. + EPS))
						throw std::logic_error("surface point with Type::Face has bary coord that sum to > 1 " + std::to_string(faceCoords));
					break;
				}
			}
		}
			...
			switch (pA.type) {
				case SurfacePointType::Vertex:
					for (Face f: pA.vertex.adjacentFaces()) {
						if (checkAdjacent(SurfacePoint(f, Vector3::zero()), pB))
							return f;
					}
					break;
		...
		inline Edge sharedEdge(const SurfacePoint& pA, const SurfacePoint& pB) {
			if (pA.type == SurfacePointType::Face || pB.type == SurfacePointType::Face)
				return Edge();
