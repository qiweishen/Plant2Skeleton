...
namespace geometrycentral {
	namespace surface {
		...
		public:
			...
			std::array<int, 3> computeVertexInsertionDataGeodesic(IntrinsicGeometryInterface& geo, Face f, const Vector3 location);
		...
		// variable names;
		std::pair<size_t, size_t> flipRoundabouts(int nij, int njk, int nki, int nil, int nlj, int nkl, size_t rki, size_t rlj, size_t dk, size_t dl);
		...
		std::vector<std::pair<SurfacePoint, double>> generateFullSingleGeodesicGeometry(ManifoldSurfaceMesh& mesh, IntrinsicGeometryInterface& geo,
																						const NormalCoordinatesCurve& curve, bool verbose = false);
