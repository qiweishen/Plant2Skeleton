namespace geometrycentral {
	namespace surface {
		...
		template<typename T>
		VertexPositionGeometry::VertexPositionGeometry(SurfaceMesh& mesh_, const Eigen::MatrixBase<T>& vMat) : VertexPositionGeometry(mesh_) {
			// sanity checks on input matrix dimensions
			GC_SAFETY_ASSERT(vMat.cols() == 3, "input must be a V x 3 matrix -- cols() should == 3");
			GC_SAFETY_ASSERT(static_cast<size_t>(vMat.rows()) == mesh_.nVertices(), "input must be a V x 3 matrix -- rows() should == nVertices()");
