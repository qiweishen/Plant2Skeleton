...
namespace geometrycentral {
	namespace surface {
		...
		struct RemeshOptions {
			...
			RemeshBoundaryCondition boundaryCondition = RemeshBoundaryCondition::Tangential;  // allowed movement of boundary vertices
		};
		...
		double smoothByLaplacian(ManifoldSurfaceMesh& mesh, VertexPositionGeometry& geom, MutationManager& mm, double stepSize = 1,
								 RemeshBoundaryCondition bc = RemeshBoundaryCondition::Tangential);
		...
		double smoothByCircumcenter(ManifoldSurfaceMesh& mesh, VertexPositionGeometry& geom, double stepSize = 1,
									RemeshBoundaryCondition bc = RemeshBoundaryCondition::Tangential);
		double smoothByCircumcenter(ManifoldSurfaceMesh& mesh, VertexPositionGeometry& geom, MutationManager& mm, double stepSize = 1,
									RemeshBoundaryCondition bc = RemeshBoundaryCondition::Tangential);
