...
namespace geometrycentral {
	namespace surface {
		...
		inline BarycentricVector::BarycentricVector(Face f) : type(BarycentricVectorType::Face), face(f), faceCoords(Vector3::zero()) {}
		...
		inline BarycentricVector::BarycentricVector(SurfacePoint pA, SurfacePoint pB) {
			...
			if (f == Face())
				throw std::invalid_argument("Input SurfacePoints of a BarycentricVector must share a face.");
		...
		inline BarycentricVector BarycentricVector::inFace(Face targetFace) const {
			switch (type) {
				...
				case BarycentricVectorType::Edge: {
					...
					he = he.next();
					Vector3 faceCoords = he.orientation() ? Vector3{ edgeCoords[1], 0., edgeCoords[0] } : Vector3{ edgeCoords[0], 0., edgeCoords[1] };
					return BarycentricVector(targetFace, faceCoords);
					break;
				}
				case BarycentricVectorType::Vertex:
					for (Vertex v: targetFace.adjacentVertices()) {
						if (v == vertex)
							return BarycentricVector(targetFace, Vector3::zero());
					}
					break;
			}

			throw std::logic_error("BarycentricVector " + std::to_string(*this) + " not adjacent to target face " + std::to_string(targetFace));
			return *this;
		}
			...
			switch (type) {
				...
				case BarycentricVectorType::Vertex:
					for (Vertex v: targetEdge.adjacentVertices()) {
						if (v == vertex)
							return BarycentricVector(targetEdge, Vector2::zero());
					}
					break;
			}

			throw std::logic_error("BarycentricVector " + std::to_string(*this) + " not adjacent to target edge " + std::to_string(targetEdge));
			return *this;
		}
		...
		inline void BarycentricVector::validate() const {
			...
			switch (type) {
				case BarycentricVectorType::Face:
					if (face == Face())
						throw std::logic_error("BarycentricVector with Type::Face has invalid face ref");
					if (!isfinite(faceCoords))
						throw std::logic_error("BarycentricVector with Type::Face has non-finite coords");
					if (abs(sum(faceCoords)) > eps)
						throw std::logic_error("BarycentricVector with Type::Face has coords that do not sum to 0 " + std::to_string(faceCoords));
					break;
				case BarycentricVectorType::Edge:
					if (edge == Edge())
						throw std::logic_error("BarycentricVector with Type::Edge has invalid edge ref");
					if (!isfinite(edgeCoords))
						throw std::logic_error("BarycentricVector with Type::Edge has non-finite coords");
					if (abs(sum(edgeCoords)) > eps)
						throw std::logic_error("BarycentricVector with Type::Edge has coords that do not sum to 0 " + std::to_string(edgeCoords));
					break;
				case BarycentricVectorType::Vertex:
					if (vertex == Vertex())
						throw std::logic_error("BarycentricVector with Type::Vertex has invalid vertex ref");
					break;
			}
		}
		...
		inline Edge sharedEdge(const BarycentricVector& u, const BarycentricVector& w) {
			if (u.type == BarycentricVectorType::Face || w.type == BarycentricVectorType::Face)
				return Edge();
		...
		inline double norm(IntrinsicGeometryInterface& geom, const BarycentricVector& v) {
			return v.norm(geom);
		}
		inline double norm2(IntrinsicGeometryInterface& geom, const BarycentricVector& v) {
			return v.norm2(geom);
		}
