...
namespace geometrycentral {
	namespace surface {
		...
		public:
			...
			std::unordered_map<SegmentID, std::tuple<Halfedge, SegmentID, SegmentID>> pathHeInfo;  // ID --> <Halfedge, prevID, nextID>
			...
			void replacePathSegment(SegmentID halfedgeNextID, SegmentAngleType angleType, const std::vector<Halfedge>& newHalfedges);
		...
		public:
			...
			FlipEdgeNetwork(ManifoldSurfaceMesh& mesh_, IntrinsicGeometryInterface& inputGeom, const std::vector<std::vector<Halfedge>>& paths,
							VertexData<bool> extraMarkedVerts = VertexData<bool>());

			// === Static initializers

			// Run Dijkstra between endpoints to initialize path
			static std::unique_ptr<FlipEdgeNetwork> constructFromDijkstraPath(ManifoldSurfaceMesh& mesh, IntrinsicGeometryInterface& geom,
																			  Vertex startVert, Vertex endVert);
			// Run Dijkstra between i'th and (i+1)'th point to initialize path
			static std::unique_ptr<FlipEdgeNetwork> constructFromPiecewiseDijkstraPath(ManifoldSurfaceMesh& mesh, IntrinsicGeometryInterface& geom,
																					   std::vector<Vertex> points, bool closed = false,
																					   bool markInterior = false);
			...
			static std::unique_ptr<FlipEdgeNetwork> constructFromEdgeSet(ManifoldSurfaceMesh& mesh, IntrinsicGeometryInterface& geom,
																		 const EdgeData<bool>& inPath, const VertexData<bool>& extraMarkedVertices);
