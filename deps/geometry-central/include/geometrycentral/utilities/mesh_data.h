...
namespace geometrycentral {
	...
	public:
		...
		MeshData(ParentMeshT& parentMesh, const Eigen::Matrix<T, Eigen::Dynamic, 1>& vector, const MeshData<E, size_t>& indexer);
...
#define GC_INTERNAL_MESHDATA_UNARY(OP)                                                   \
	...
	MeshData<E, decltype(OP std::declval<T>())> operator OP(const MeshData<E, T>& lhs) { \
		...
		for (Eigen::Index i = 0; i < lhs.raw().rows(); i++)                              \
			result.raw()[i] = OP lhs.raw()[i];                                           \
		return result;                                                                   \
	}
...
#define GC_INTERNAL_MESHDATA_BINARY_BASIC(OP)                                                                                         \
	template<typename E, typename T, typename U>                                                                                      \
	MeshData<E, decltype(std::declval<T>() OP std::declval<U>())> operator OP(const MeshData<E, T>& lhs, const MeshData<E, U>& rhs) { \
		checkMeshCompatible(lhs, rhs);                                                                                                \
		MeshData<E, decltype(std::declval<T>() OP std::declval<U>())> result(*lhs.getMesh());                                         \
		for (Eigen::Index i = 0; i < lhs.raw().rows(); i++)                                                                           \
			result.raw()[i] = lhs.raw()[i] OP rhs.raw()[i];                                                                           \
		return result;                                                                                                                \
	}                                                                                                                                 \
	template<typename E, typename T, typename U>                                                                                      \
	...
	MeshData<E, decltype(std::declval<T>() OP std::declval<U>())> operator OP(const T & lhs, const MeshData<E, U>& rhs) {             \
		...
		for (Eigen::Index i = 0; i < rhs.raw().rows(); i++)                                                                           \
			result.raw()[i] = lhs OP rhs.raw()[i];                                                                                    \
		return result;                                                                                                                \
	}
...
#define GC_INTERNAL_MESHDATA_BINARY_EQUALS_ASSIGN(OP)                                                                               \
	...
	template<typename E, typename T, typename U>                                                                                    \
	MeshData<E, decltype(std::declval<T>() OP std::declval<U>())>& operator OP##=(MeshData<E, T>& lhs, const MeshData<E, U>& rhs) { \
		checkMeshCompatible(lhs, rhs);                                                                                              \
		for (Eigen::Index i = 0; i < lhs.raw().rows(); i++)                                                                         \
