...
namespace geometrycentral {
	...
	template<typename E, typename T>
	MeshData<E, T>::MeshData(ParentMeshT& parentMesh, const Eigen::Matrix<T, Eigen::Dynamic, 1>& vector) : MeshData(parentMesh) {
		fromVector(vector);
	}

	template<typename E, typename T>
	MeshData<E, T>::MeshData(ParentMeshT& parentMesh, const Eigen::Matrix<T, Eigen::Dynamic, 1>& vector, const MeshData<E, size_t>& indexer) :
		MeshData(parentMesh) {
		fromVector(vector, indexer);
	}

	template<typename E, typename T>
	MeshData<E, T>::MeshData(const MeshData<E, T>& other) : mesh(other.mesh), defaultValue(other.defaultValue), data(other.data) {
		registerWithMesh();
	}

	template<typename E, typename T>
	MeshData<E, T>::MeshData(MeshData<E, T>&& other) noexcept : mesh(other.mesh), defaultValue(other.defaultValue), data(std::move(other.data)) {
		registerWithMesh();
	}
	...
	void MeshData<E, T>::fromVector(const Eigen::Matrix<T, Eigen::Dynamic, 1>& vector) {
		if ((size_t) vector.rows() != nElements<E>(mesh))
	...
	inline MeshData<E, T> MeshData<E, T>::reinterpretTo(ParentMeshT& targetMesh) const {
		GC_SAFETY_ASSERT(nElements<E>(mesh) == nElements<E>(&targetMesh), "meshes must have same number of elements to reinterpret");
