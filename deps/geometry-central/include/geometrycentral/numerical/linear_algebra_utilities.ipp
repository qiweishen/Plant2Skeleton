...
SparseMatrix<T> verticalStack(const std::vector<SparseMatrix<T>, Eigen::aligned_allocator<SparseMatrix<T>>>& mats) {
	if (mats.size() == 0)
		throw std::logic_error("must have at least one matrix to stack");
	...
	for (const SparseMatrix<T>& mat: mats) {
		if (mat.cols() != nCols)
			throw std::logic_error("all matrices must have same column size");
...
SparseMatrix<T> horizontalStack(const std::vector<SparseMatrix<T>, Eigen::aligned_allocator<SparseMatrix<T>>>& mats) {
	...
	for (const SparseMatrix<T>& mat: mats) {
		if (mat.rows() != nRows)
			throw std::logic_error("all matrices must have same row size");
	...
	for (int k = 0; k < m.outerSize(); ++k) {
		for (typename SparseMatrix<T>::InnerIterator it(m, k); it; ++it) {
			if (!isfinite(it.value())) {
				...
				msg << "checkFinite() failure: Non-finite matrix entry [" << it.row() << "," << it.col() << "] = " << it.value();
		...
		for (typename SparseMatrix<T>::InnerIterator it(m, k); it; ++it) {
			...
			if (std::abs(thisVal - otherVal) > eps) {
				...
				msg << "checkSymmetric() error. Non-symmtric matrix entry at [" << it.row() << "," << it.col() << "]."
					<< "    [" << it.row() << "," << it.col() << "] = " << thisVal << "    [" << it.col() << "," << it.row() << "] = " << otherVal;
				// std::cerr << msg.str() << std::endl;
				throw std::logic_error(msg.str());
			}
		}
...
template<typename T>
BlockDecompositionResult<T> blockDecomposeSquare(const SparseMatrix<T>& m, const Vector<bool>& Aset, bool buildBuildBside) {
	if (m.rows() != m.cols())
		throw std::logic_error("blockDecomposeSquare must be called on square matrix");
...
SparseMatrix<T> loadSparseMatrix(std::istream& in) {
	...
	if (cols > 0 && (maxCol >= cols || maxRow >= rows)) {
		std::cout << "Error: stated cols (" << cols << ") and rows (" << rows << ") are insufficient to fit all entries." << std::endl;
