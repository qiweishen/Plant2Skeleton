...
namespace tool {
    ...
    // TODO: Consider to use hapPLY for PLY file reading and writing, library is available at SkelSeg/deps/geometry-central/deps/happly
    namespace io {
    ...
    namespace utility {
        ...
        std::vector<int> FindUpperOutlierBySTD(const std::vector<double> &data) {
            ...
            if (n == 0) return {}; // Handle empty data case
            // Calculate the mean
        ...
        std::vector<size_t> RandomDownSample(const std::shared_ptr<Eigen::MatrixXd> &cloudPtr, int num_samples) {
            if (num_samples <= 0) {
                std::cerr << "Number of samples should be larger than 0." << std::endl;
            } else if (num_samples >=
                       cloudPtr->rows()) { // Modified condition to cover all cases where num_samples >= num_points
                if (num_samples > cloudPtr->rows()) {
                    std::cerr << "Number of samples should be smaller than the total point size: " << cloudPtr->rows()
                              << std::endl;
                }
            ...
            std::vector<size_t> random_indices(cloudPtr->rows());
            std::iota(std::begin(random_indices), std::end(random_indices), (size_t) 0);
            {
                std::lock_guard<std::mutex> lock(*GetMutex());
                std::shuffle(random_indices.begin(), random_indices.end(),
                             *GetEngine());
            }
            random_indices.resize((int) (num_samples));
...
#pragma omp critical
    ...
    namespace visualize {
        ...
        void DrawTangentPoints(const std::string &title, const std::shared_ptr<Eigen::MatrixXd> &cloudPtr,
            ...
            geometrycentral::pointcloud::PointData<std::vector<std::array<
                    geometrycentral::pointcloud::Point, 3>>> localTriPoint = geometrycentral::pointcloud::buildLocalTriangulations(
                    *gc_cloudPtr, *gc_geom, true);
...
//            psCloud_2->addToGroup(*group);
        ...
        void DrawTwoTangentPoints(const std::string &title, const std::shared_ptr<Eigen::MatrixXd> &origianl_cloudPtr,
                                  ...
                                  const int k, const int center_index) {
            std::shared_ptr<geometrycentral::pointcloud::PointCloud> gc_origianl_cloudPtr = std::make_shared<geometrycentral::pointcloud::PointCloud>(
                    origianl_cloudPtr->rows());
            std::shared_ptr<geometrycentral::pointcloud::PointPositionGeometry> gc_origianl_geom = std::make_shared<geometrycentral::pointcloud::PointPositionGeometry>(
                    *gc_origianl_cloudPtr);
            ...
            std::shared_ptr<geometrycentral::pointcloud::PointCloud> gc_contracted_cloudPtr = std::make_shared<geometrycentral::pointcloud::PointCloud>(
                    contracted_cloudPtr->rows());
            std::shared_ptr<geometrycentral::pointcloud::PointPositionGeometry> gc_contracted_geom = std::make_shared<geometrycentral::pointcloud::PointPositionGeometry>(
                    *gc_contracted_cloudPtr);
            ...
            geometrycentral::pointcloud::PointData<std::vector<std::array<
                    geometrycentral::pointcloud::Point, 3>>> localTriPoint = geometrycentral::pointcloud::buildLocalTriangulations(
                    *gc_origianl_cloudPtr, *gc_origianl_geom, true);
            std::vector<std::array<geometrycentral::pointcloud::Point, 3>> localTri = localTriPoint[center_index];
            std::vector<std::vector<size_t>> tris;
            tris.reserve(localTri.size());
            for (const std::array<geometrycentral::pointcloud::Point, 3> &tri: localTri) {
