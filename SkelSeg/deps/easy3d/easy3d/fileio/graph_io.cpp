...
namespace easy3d {
    ...
    Graph* GraphIO::load(const std::string& file_name)
	{
        ...
        if (ext == "ply")
            ...
            LOG(ERROR) << "unknown file format: " << ext << ". Only Binary format is supported for Graph" << ext;
            return nullptr;
        }
    ...
    bool GraphIO::save(const std::string& file_name, const Graph* graph)
	{
        ...
        if (ext == "ply" || ext.empty()) {
            ...
            LOG(ERROR) << "unknown file format: " << ext << ". Only Binary format is supported for Graph";
            success = false;
        }
