...
Eigen::MatrixXd LOPCalibrate(const Eigen::MatrixXd &cloud, const Eigen::MatrixXd &skeleton_cloud, const nlohmann::json &config) {
	...
	double h = config["Preprocess"]["Normalize_AABB_Length"].get<double>() * config["Skeleton_Building"]["LOP_Sphere_Radius_Ratio"].get<double>();

	std::atomic<bool> has_nan{ false };
	std::atomic<int> nan_index{ -1 };

#pragma omp parallel for default(none) shared(n, skeleton_cloud, kdtree, h, new_skeleton_cloud, cloud, has_nan, nan_index, Eigen::Dynamic)
	for (int i = 0; i < n; ++i) {
		...
		if (original_point_cloud_indices.empty()) {
			new_skeleton_cloud.row(i) = skeleton_cloud.row(i);
			continue;
		}
	...
	if (has_nan) {
		Logger::Instance().Log(fmt::format("NaN in LOP calibration at index {}", nan_index.load()), LogLevel::ERROR);
	}

	double elapsed = timer.elapsed<Timer::TimeUnit::Seconds>();
	Logger::Instance().Log(fmt::format("LOP-based Calibration complete! Elapsed time: {:.6f}s", elapsed), LogLevel::INFO, IndentLevel::ONE, true,
						   false);
