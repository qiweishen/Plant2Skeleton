...
namespace tool {
	namespace preprocess {
		void PreparePointCloud(Eigen::MatrixXd &final_cloud, nlohmann::json &config) {
			...
			if (!with_labels) {
				// No labels
				std::vector<easy3d::vec3> cloud_vertices = io::LoadPointCloud(config, false);
				...
				if (std::unique_ptr<easy3d::PointCloud> loaded_cloud(easy3d::PointCloudIO::load(ply_path.string())); loaded_cloud) {
					...
					Logger::Instance().Log(fmt::format("Failed to load point cloud from file ({}) by Easy3D", ply_path.string()), LogLevel::ERROR);
				}
				double elapsed = timer_1.elapsed<Timer::TimeUnit::Seconds>();
				Logger::Instance().Log(fmt::format("Original Point Cloud (#vertex: {}) has been loaded by Easy3D! Elapsed time: {:.6f}s",
...
#pragma omp parallel for default(none) shared(num_samples, points, final_cloud_map)
		...
		namespace internal {
			void UniformDownSample(easy3d::PointCloud &cloud, unsigned int num_samples, bool with_log) {
				...
				if (original_num_vertices < num_samples) {
					Logger::Instance().Log(
							fmt::format("The Original Point Cloud point size (#vertex {}) is less than the downsample number (#num {})",
	...
	namespace io {
		std::vector<easy3d::vec3> LoadPointCloud(nlohmann::json &config, bool backup) {
			Timer timer;
			...
			if (backup) {
				...
				Logger::Instance().Log(
						fmt::format(
								"Original Point Cloud (#vertex: {}) has been loaded and a backup PLY file has been generated! Elapsed time: {:.6f}s",
		...
		void SavePointCloudToPLY(const Eigen::MatrixXd &cloud, const std::filesystem::path &file_path) {
			...
			if (!ply_ofs) {
				Logger::Instance().Log(fmt::format("Failed to open file ({}) for writing", file_path.string()), LogLevel::ERROR);
			}
		...
		void SavePointCloudToPLY(const Eigen::MatrixXd &cloud, const std::filesystem::path &file_path, const std::vector<double> &property,
			...
			if (num_pts != property.size()) {
				Logger::Instance().Log(fmt::format("Point cloud (#vertex {}) or property (#size {}) has invalid sizes", num_pts, property.size()),
									   LogLevel::ERROR);
			}
			std::vector<plywoot::Comment> comments;
			comments.emplace_back(2, PLY_COMMENT);
		...
		void SavePointCloudToPLY(const Eigen::MatrixXd &cloud, const std::filesystem::path &file_path, const std::vector<int> &label_one,
			...
			if (num_pts != label_one.size() || num_pts != label_two.size()) {
				Logger::Instance().Log(fmt::format("Point cloud (#vertex {}), label one (#size {}), or label two (#size {}) has invalid sizes",
		...
		void SaveSkeletonGraphToPLY(const Boost_Graph &graph, const std::filesystem::path &file_path, const std::vector<int> &vertex_semantic_labels,
			...
			for (auto ei = boost::edges(graph).first; ei != boost::edges(graph).second; ++ei) {
				...
				if (source_instance_label == target_instance_label) {
					...
					Logger::Instance().Log(fmt::format("Error assigning edge label (#source instance label {}, #target instance label {}).",
		...
		namespace internal {
			...
			void FormatPointCloudFromXYZTXT(nlohmann::json &config) {
				if (config["Input_Settings"]["Labels_Names"]["TXT_XYZ_Format"]["Individual_Labels_File"].get<bool>()) {
					std::filesystem::path file_path = config["Input_Settings"]["Point_Cloud_File_Path"].get<std::filesystem::path>();
					std::filesystem::path output_folder_path = config["Output_Settings"]["Output_Folder_Path"].get<std::filesystem::path>();
					std::vector<Eigen::Vector3d> cloud_vertices = LoadDataFromTXTXYZ<Eigen::Vector3d>(file_path, 3);
					std::filesystem::path semantic_label_file_path =
							config["Input_Settings"]["Labels_Names"]["TXT_XYZ_Format"]["Labels_File_Paths"]["Semantic_Label_File_Path"]
									.get<std::filesystem::path>();
					std::vector<int> semantic_labels = LoadDataFromTXTXYZ<int>(semantic_label_file_path, 1);
					std::filesystem::path instance_label_file_path =
							config["Input_Settings"]["Labels_Names"]["TXT_XYZ_Format"]["Labels_File_Paths"]["Instance_Label_File_Path"]
									.get<std::filesystem::path>();
					std::vector<int> instance_labels = LoadDataFromTXTXYZ<int>(instance_label_file_path, 1);
					if (semantic_labels.size() != cloud_vertices.size() || instance_labels.size() != cloud_vertices.size()) {
						Logger::Instance().Log(fmt::format("The number of labels (#semantic {}, #instance {}) does not match the number of vertices "
					...
					std::filesystem::path file_path = config["Input_Settings"]["Point_Cloud_File_Path"].get<std::filesystem::path>();
					std::filesystem::path output_folder_path = config["Output_Settings"]["Output_Folder_Path"].get<std::filesystem::path>();
					int total_columns = config["Input_Settings"]["Labels_Names"]["TXT_XYZ_Format"]["Total_Columns"].get<int>();
					std::vector<Eigen::VectorXd> data = LoadDataFromTXTXYZ<Eigen::VectorXd>(file_path, total_columns);
					int semantic_label_index = config["Input_Settings"]["Labels_Names"]["TXT_XYZ_Format"]["Semantic_Labels_Index"].get<int>();
					int instance_label_index = config["Input_Settings"]["Labels_Names"]["TXT_XYZ_Format"]["Instance_Labels_Index"].get<int>();
				...
				while (start < data_view.size()) {
					...
					if constexpr (std::is_same_v<T, int> || std::is_same_v<T, double>) {
						...
						if (count == dim) {
							...
							Logger::Instance().Log(fmt::format("Dimension does not match, expected {}, got {}", dim, count), LogLevel::ERROR);
						}
					}
					start = end + 1;
				}
	...
	namespace utility {
		std::tuple<Eigen::Vector3d, double> Normalize(Eigen::MatrixXd &cloud, double diagonal_length) {
			if (cloud.rows() == 0 || cloud.cols() != 3) {
				Logger::Instance().Log(fmt::format("Invalid Point Cloud dimensions ({}, {})", cloud.rows(), cloud.cols()), LogLevel::ERROR);
			}
		...
		std::vector<T> Matrix2Vector(const Eigen::MatrixXd &mat) {
			...
			if (num_cols != 3) {
				Logger::Instance().Log(fmt::format("Invalid Point Cloud dimensions ({}, {})", num_rows, num_cols), LogLevel::ERROR);
			}
...
#pragma omp parallel for default(none) shared(cloud_vertices, kdtree, radius, result)
		...
		std::tuple<Eigen::MatrixXd, std::vector<size_t> > FarthestPointDownSample(const Eigen::MatrixXd &cloud, size_t num_samples) {
			Logger::Instance().AddLine(LogLine::DASH);
			Logger::Instance().Log(fmt::format("Start downsample the Point Cloud to {} by farthest point downsample", num_samples), LogLevel::INFO);
			Timer timer;
			if (num_samples <= 0) {
				Logger::Instance().Log("The downsample number must be greater than 0", LogLevel::ERROR);
			}
			...
			if (num_points < num_samples) {
				Logger::Instance().Log(
						fmt::format("The Point Cloud point size (#vertex {}) is less than the downsample number (#num {})", num_points, num_samples),
						LogLevel::ERROR);
			...
			double elapsed = timer.elapsed<Timer::TimeUnit::Seconds>();
			Logger::Instance().Log(fmt::format("Farthest point downsample has been completed! Elapsed time: {:.6f}s", elapsed), LogLevel::INFO,
		...
		std::vector<int> NearestProjectFromBoostVertices(const Boost_Graph &graph, const Eigen::MatrixXd &cloud,
			...
			double elapsed = timer.elapsed<Timer::TimeUnit::Seconds>();
			Logger::Instance().Log(fmt::format("Stem-Leaf detection has been completed! Elapsed time: {:.6f}s", elapsed), LogLevel::INFO,
								   IndentLevel::ONE, true, false);
			return result;
		}
	}  // namespace utility
