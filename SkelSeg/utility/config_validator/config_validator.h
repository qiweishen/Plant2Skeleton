...
namespace configvalidator {
    ...
    std::optional<std::string> ValidateInputSettings(const nlohmann::json &input_settings) {
        ...
        if (batch_processing) {
            ...
            if (!input_settings.contains("Batch_Processing_Folder_Path") ||
                !input_settings["Batch_Processing_Folder_Path"].is_string()) {
            ...
            try {
                if (!std::filesystem::is_directory(folder_path)) {
                    return std::format("Invalid 'Batch_Processing_Folder_Path': '{}' is not a directory.",
                                       folder_path.string());
                }
            ...
            if (!input_settings.contains("Point_Cloud_File_Extension") ||
                !input_settings["Point_Cloud_File_Extension"].is_string()) {
        ...
        if (!batch_processing) {
            if (!input_settings.contains("Point_Cloud_File_Path") ||
                !input_settings["Point_Cloud_File_Path"].is_string()) {
        ...
        if (input_settings["With_Labels"].get<bool>()) {
            ...
            if (batch_processing) {
                ...
                file_extension = ToLower(
                        input_settings["Point_Cloud_File_Path"].get<std::filesystem::path>().extension());
            }
            // Check "Labels_Names"
    ...
    std::optional<std::string> ValidateTXTXYZFormat(const nlohmann::json &txt_xyz_format) {
        if (!txt_xyz_format.contains("Individual_Labels_File") ||
            !txt_xyz_format["Individual_Labels_File"].is_boolean()) {
        ...
        if (individualLabelsFile) {
            ...
            if (!txt_xyz_format.contains("Semantic_Labels_Index") ||
                !txt_xyz_format["Semantic_Labels_Index"].is_number_integer() ||
            ...
            if (!txt_xyz_format.contains("Instance_Labels_Index") ||
                !txt_xyz_format["Instance_Labels_Index"].is_number_integer() ||
    ...
    ValidateConstraintLaplacianOperator(const nlohmann::json &constraint_laplacian_operator) {
        if (!constraint_laplacian_operator.contains("Initial_k") ||
            !constraint_laplacian_operator["Initial_k"].is_number_integer()) {
            return "Missing or invalid 'Initial_k' in 'Constraint_Laplacian_Operator'.";
        }

        if (!constraint_laplacian_operator.contains("Delta_k") ||
            !constraint_laplacian_operator["Delta_k"].is_number_integer()) {
            return "Missing or invalid 'Delta_k' in 'Constraint_Laplacian_Operator'.";
        }

        if (!constraint_laplacian_operator.contains("Max_k") ||
            !constraint_laplacian_operator["Max_k"].is_number_integer()) {
    ...
    std::optional<std::string> ValidateAdaptiveContraction(const nlohmann::json &adaptive_contraction) {
        if (!adaptive_contraction.contains("Smooth_Sigma_Threshold") ||
            !adaptive_contraction["Smooth_Sigma_Threshold"].is_number()) {
            return "Missing or invalid 'Smooth_Sigma_Threshold' in 'Adaptive_Contraction'.";
        }

        if (!adaptive_contraction.contains("Sigma_Sphere_Radius_Ratio") ||
            !adaptive_contraction["Sigma_Sphere_Radius_Ratio"].is_number()) {
            return "Missing or invalid 'Sigma_Sphere_Radius_Ratio' in 'Adaptive_Contraction'.";
        }

        if (!adaptive_contraction.contains("Sigma_KNN_Search") ||
            !adaptive_contraction["Sigma_KNN_Search"].is_number_integer()) {
            return "Missing or invalid 'Sigma_KNN_Search' in 'Adaptive_Contraction'.";
        }

        if (!adaptive_contraction.contains("Max_Distance_Ratio") ||
            !adaptive_contraction["Max_Distance_Ratio"].is_number()) {
    ...
    std::optional<std::string> ValidateTerminateCondition(const nlohmann::json &terminate_condition) {
        if (!terminate_condition.contains("Max_Iteration") ||
            !terminate_condition["Max_Iteration"].is_number_integer()) {
            return "Missing or invalid 'Max_Iteration' in 'Terminate_Condition'.";
        }

        if (!terminate_condition.contains("Convergence_Threshold") ||
            !terminate_condition["Convergence_Threshold"].is_number()) {
    ...
    std::optional<std::string> ValidateSkeletonBuilding(const nlohmann::json &skeleton_building) {
        if (!skeleton_building.contains("Down_Sample_Number") ||
            !skeleton_building["Down_Sample_Number"].is_number_integer()) {
            return "Missing or invalid 'Down_Sample_Number' in 'Skeleton_Building'.";
        }

        if (!skeleton_building.contains("LOP_Sphere_Radius_Ratio") ||
            !skeleton_building["LOP_Sphere_Radius_Ratio"].is_number()) {
    ...
    std::optional<std::string> ValidateOutputSettings(const nlohmann::json &output_settings) {
            ...
            if (!std::filesystem::exists(output_folder_path)) {
                ...
                if (has_contents) {
                    ...
                    for (int attempt = 0; attempt < max_attempts; ++attempt) {
                        std::cout
                                << "Enter 'y' to terminate the program, or 'n' to continue and delete all contents: [y]/n"
                                << std::endl;
        ...
        if (!output_settings.contains("Output_File_DataFormat") ||
            !output_settings["Output_File_DataFormat"].is_string()) {
            return "Missing or invalid 'Output_File_DataFormat' in 'Output_Settings'.";
        }

        std::string data_format = output_settings["Output_File_DataFormat"].get<std::string>();
        if (data_format != "Binary" && data_format != "ASCII") {
            return "Invalid 'Output_File_DataFormat' in 'Output_Settings'. Supported formats: Binary, ASCII.";
        }
