...
/**
 * This file is slightly modified from https://github.com/isl-org/Open3D/blob/main/cpp/open3d/utility/Random.h
 * to support the tool::utility::RandomDownSample function.
 */
...
public:
    ...
    UniformIntGenerator(const T low, const T high) : distribution_(low, high) {
        if (low < 0) {
            std::cerr << "low must be > 0, but got " << low << "." << std::endl;
        }
        if (low >= high) {
            std::cerr << "low must be < high, but got low=" << low << " and high=" << high << "." << std::endl;
        }
    }
...
public:
    ...
    NormalGenerator(const T mean = 0.0, const T stddev = 1.0)
        ...
        if (stddev <= 0) {
            std::cerr << "stddev must be > 0, but got " << stddev << "." << std::endl;
        }
    }
...
public:
    ...
    DiscreteGenerator(InputIt first, InputIt last)
        ...
        if (first > last) {
            std::cerr << "first must be <= last." << std::endl;
        }
    }
