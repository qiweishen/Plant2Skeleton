...
//        }
    ...
    namespace visualize {
        void DrawPointClouds(const std::string &group_title,
                             const std::vector <std::pair<std::string, std::shared_ptr < Eigen::MatrixXd>>

        > &cloudsPairs) {
        if (

        polyscope::isInitialized()
...
psCloud->
setPointRenderMode(polyscope::PointRenderMode::Sphere);
...
//
    ...
    psCloud_1->setPointColor(glm::vec3(49.0 / 255.0, 49.0 / 255.0, 129.0 / 255.0));
    psCloud_1->addToGroup(*group);
    polyscope::PointCloud *psCloud_2 = polyscope::registerPointCloud("pc_2", std::vector < geometrycentral::Vector3 >
