...
int main(int argc, char **argv) {
	bool snip_segmentation_error = false;
	...
	if (!snip_segmentation_error) {
		...
		for (int i = 0; i < ply_input_folders.size(); i++) {
			...
			ok = snipping::snapshot(os, original_input_file, skeleton_input_file, output, result_points_cfg, skeleton_points_cfg, lines_cfg,
									camera_cfg);
			if (ok) {
		...
		semantic_result_cfg.colormap = "colormap_twocolors";
		semantic_result_cfg.num_stripes = 2;
		for (int i = 0; i < ply_input_folders.size(); i++) {
			std::filesystem::path original_input_file = std::filesystem::path(ply_input_folders[i]) / (sample_names[i] + "_Result.ply");
			std::filesystem::path skeleton_input_file = "";
		...
		for (int i = 0; i < ply_input_folders.size(); i++) {
			...
			ok = snipping::snapshot(os, original_input_file, skeleton_input_file, output, original_points_cfg, semantic_skeleton_points_cfg,
									lines_cfg, camera_cfg);
			if (ok) {
				std::cout << "Snapshot saved to: " << output_file << std::endl;
			} else {
				std::cerr << "Failed to render snapshot." << std::endl;
		...
		for (int i = 0; i < ply_input_folders.size(); i++) {
			...
			ok = snipping::snapshot(os, original_input_file, skeleton_input_file, output, original_points_cfg, instance_skeleton_points_cfg,
									lines_cfg, camera_cfg);
			if (ok) {
				std::cout << "Snapshot saved to: " << output_file << std::endl;
			} else {
		...
		original_result_cfg.point_size = 2.0f;
		...
		easy3d::vec3 uniform_skeleton_color = easy3d::vec3(234, 0, 141) / 255.0f;
		...
		result_points_cfg.point_size = 3.0f;
		result_points_cfg.impostor = easy3d::PointsDrawable::PLAIN;
		result_points_cfg.coloring = easy3d::State::COLOR_PROPERTY;
		// Placeholder
		snipping::PointsConfig skeleton_points_cfg;
		for (const auto &entry: std::filesystem::directory_iterator(skelseg_output_folder)) {
			if (entry.is_regular_file()) {
				if (entry.path().extension() == ".ply") {
					std::filesystem::path original_input_file = entry.path().string();
					std::filesystem::path skeleton_input_file = "";
					std::filesystem::path output_file = img_output_folder + "/" + entry.path().stem().string() + "_comparison.png";
