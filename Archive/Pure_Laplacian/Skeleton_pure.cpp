...
void Skeleton::Convert2Matrix() {
    for (int i = 0; i < cloudPtr_->points_.size(); ++i) {
        ptsPtr_->row(i) = cloudPtr_->points_.at(i);
    }
}
...
bool Skeleton::FindOneRingNeighbors(std::shared_ptr<PointCloud>& cloudPtr) {
    ...
    #pragma omp parallel for shared(n, cloudPtr, status)
    for (int i = 0; i < n; ++i) {
        ...
        for (size_t j = 0; j < k_neighbors_idx.size(); ++j) {
...
void Skeleton::ComputeLaplacianMatrix(const std::shared_ptr<Eigen::MatrixXd>& ptsPtr) {
    auto start = std::chrono::high_resolution_clock::now();
...
void Skeleton::InitializeParameters() {
    // Compute initial WL & WH diagonal matrix
    Eigen::VectorXd mean_extents = Skeleton::GetNeighborhoodExtent(ptsPtr_, "mean");
    std::cout << "Init Contract weight: " << 1.0 / (5.0 * mean_extents.mean()) << std::endl;
    WL_Ptr_->fill(1.0 / (5.0 * mean_extents.mean()));
    for (int i = 0; i < ptsPtr_->rows(); ++i) {
        WH_Ptr_->coeffRef(i, i) = 1.0;
    }

    Skeleton::ComputeLaplacianMatrix(ptsPtr_);
}
...
std::shared_ptr<Eigen::MatrixXd> Skeleton::ContractionIteration() {
    const int n = static_cast<int>(ptsPtr_->rows());
    ...
    if (!FindOneRingNeighbors(cloudPtr_)) {
        std::cerr << "Touch the CGAL precision limit! Check original data." << std::endl;
    ...
    for (int i = 0; i < iteration_time_ - 1; ++i) {
        ...
        // Update WL & WH, and check if them exceed the maximum value
        for (int j = 0; j < n; ++j) {
            ...
            double temp_WH = WC_ * extents(j) / new_extents(j);
            if (temp_WH > 10000) {
                WH_Ptr_->coeffRef(j, j) = 10000;
            } else {
                WH_Ptr_->coeffRef(j, j) = temp_WH;
            }
        }

        // Update Laplacian matrix
        Skeleton::ComputeLaplacianMatrix(cptsPtr);
        ...
        contraction_history.emplace_back(contraction_rate);
        std::cout << "Neighborhood extent has been updated! Current extent is: " << new_extents.sum() << "; " << "the contract ratio is: " << contraction_rate << std::endl;
        if (contraction_history[i] - contraction_history.back() <= 0.01 || std::isnan(contraction_history.back())) { // 0.01 | 0.0
